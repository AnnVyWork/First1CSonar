...
Процедура УправлениеЭлементамиФормы(Форма)
	...
	Элементы.ГруппаСведенияОБанкеДляРасчетов.Доступность = Форма.ИспользуетсяБанкДляРасчетов;
	
	Элементы.СпособУказанияРеквизитовБанка.Доступность = Не ЗарубежныйСчет;
	Элементы.СпособУказанияРеквизитовБанкаДляРасчетов.Доступность = Объект.Валюта = Форма.НациональнаяВалюта;
	
	Элементы.ГруппаРеквизитыБанка.Доступность = Объект.РучноеИзменениеРеквизитовБанка;
	Элементы.ГруппаРеквизитыБанкаДляРасчетов.Доступность = Объект.РучноеИзменениеРеквизитовБанкаДляРасчетов;
	
	Если Не Объект.Зарубежный Тогда
		Элементы.СтраницыКодыБанка.ТекущаяСтраница = Элементы.СтраницаКодыБанкаРоссийскогоСчета;
	Иначе
		Элементы.СтраницыКодыБанка.ТекущаяСтраница = Элементы.СтраницаКодыБанкаЗарубежногоСчета;
	КонецЕсли;
	
	Если Объект.Валюта = Форма.НациональнаяВалюта Тогда
		Элементы.СтраницыКодыБанкаДляРасчетов.ТекущаяСтраница = Элементы.СтраницаКодыБанкаДляРасчетовРублевогоСчета;
	Иначе
		Элементы.СтраницыКодыБанкаДляРасчетов.ТекущаяСтраница = Элементы.СтраницаКодыБанкаДляРасчетовВалютногоСчета;
	КонецЕсли;
...
Процедура ВывестиВажноеСообщение()
	...
	Если ДеятельностьБанкаПрекращена Тогда
		...
		ПараметрыВажногоСообщения.Идентификатор = "ПояснениеНедействительногоБанка";
		ПараметрыВажногоСообщения.Сообщение = РаботаСБанками.ПояснениеНедействительногоБанка(Объект.Банк);
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ПояснениеНедействительногоБанка");
	КонецЕсли;
	
	Если ДеятельностьБанкаНепрямыхРасчетовПрекращена Тогда
		ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыВажногоСообщения.Скрываемый = Ложь;
		ПараметрыВажногоСообщения.Идентификатор = "ПояснениеНедействительногоБанкаДляРасчета";
		ПараметрыВажногоСообщения.Сообщение = РаботаСБанками.ПояснениеНедействительногоБанка(Объект.БанкДляРасчетов);
		ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Деятельность банка прекращена'");
		
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.ГруппаВажноеСообщениеДляРасчетов);
		ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму(ЭтотОбъект);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ПояснениеНедействительногоБанкаДляРасчета");
	КонецЕсли;
