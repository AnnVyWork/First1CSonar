	...
	Иначе
		...
		ТекстСообщения = ТекстСообщения + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Символы.ПС
			...
			+ НСтр("ru = 'Рекомендуется обновить версию платформы 1С:Предприятия. Новая версия платформы содержит исправления ошибок,
				...
				|
				|Версия платформы, необходимая для работы в программе: %2, рекомендуемая: %3 или выше, текущая: %4.'"),
			ТекущаяВерсияПлатформы,
...
Функция СохранитьНастройкиОбновленияИНачатьПроверкуНаличияОбновления(Знач НастройкиОбновления = Неопределено) Экспорт
	...
	Если ОписаниеЗадания <> Неопределено
		И ОписаниеЗадания.Статус = "Выполнено" Тогда
...
//
// Параметры:
//  Расписание - Структура - расписание в виде структуры.
//  ДатаПроверки - Дата - дата, для которой нужно проверить расписание.
//  ДатаПоследнейПроверки - Дата - дата начала последнего запуска регламентного задания.
//
// Возвращаемое значение:
//  Булево - Истина, если расписание требует выполнение проверки наличия обновления.
//
Функция РасписаниеТребуетВыполнение(Знач Расписание, Знач ДатаПроверки, Знач ДатаПоследнейПроверки) Экспорт
	
	РасписаниеОбъект = ОбщегоНазначенияКлиентСервер.СтруктураВРасписание(Расписание);

	Возврат РасписаниеОбъект.ТребуетсяВыполнение(ДатаПроверки, ДатаПоследнейПроверки);
