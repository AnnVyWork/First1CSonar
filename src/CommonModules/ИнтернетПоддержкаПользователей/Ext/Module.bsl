...
// Обработка события перемещения информационной базы.
//
// Параметры:
//  ВидИзменения - Строка - вид изменения расположения:
//   - "Копия" - информационная база была скопирована.
//   - "Перемещена" - информационная база была перемещена.
//
Процедура ПриИзмененииРасположенияБазы(ВидИзменения) Экспорт
...
Функция ПравоЗаписиПараметровИПП() Экспорт
	...
	Возврат Пользователи.ЭтоПолноправныйПользователь(, , Ложь)
		Или Пользователи.РолиДоступны("ПодключениеИнтернетПоддержки",
			,
			Ложь);
...
#Область УстановкаКартинокСостояний

Процедура УстановитьСостояниеУспешно(Элемент) Экспорт
	
	Элемент.Картинка = БиблиотекаКартинок.ВажноеСообщениеУспешно;
	Элемент.ЦветКартинки = ЦветаПалитры.Зеленый;
	
КонецПроцедуры

Процедура УстановитьСостояниеПредупреждение(Элемент) Экспорт
	
	Элемент.Картинка = БиблиотекаКартинок.ВажноеСообщениеПредупреждение;
	Элемент.ЦветКартинки = ЦветаПалитры.Желтый;
	
КонецПроцедуры

Процедура УстановитьСостояниеОшибка(Элемент) Экспорт
	
	Элемент.Картинка = БиблиотекаКартинок.ВажноеСообщениеОшибка;
	Элемент.ЦветКартинки = ЦветаПалитры.Красный;
	
КонецПроцедуры

Процедура УстановитьСостояниеИнформация(Элемент) Экспорт
	
	Элемент.Картинка = БиблиотекаКартинок.ВажноеСообщениеИнформация;
	Элемент.ЦветКартинки = ЦветаПалитры.Голубой;
...
Функция УслугаПодключена(ИдентификаторУслуги, ЗначениеРазделителя = Неопределено) Экспорт
	...
	Иначе
		...
		Если ТребуетсяРазделение Тогда
			...
			КонецЕсли;
			МодульРаботаВМоделиСервисаБИП = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервисаБИП");
			МодульРаботаВМоделиСервисаБИП.ВойтиВОбластьДанных(ЗначениеРазделителя);
		КонецЕсли;
		...
		Если ТребуетсяРазделение Тогда
			МодульРаботаВМоделиСервисаБИП.ВыйтиИзОбластиДанных(Ложь);
		КонецЕсли;
...
// Настраивает командную панель мастера.
//
// Параметры:
//  КнопкаДалее - КнопкаФормы, Неопределено - элемент формы кнопки "Далее".
//  КнопкаНазад - КнопкаФормы, Неопределено - элемент формы кнопки "Назад".
//
Процедура НастроитьКоманднуюПанельМастера(
		КнопкаДалее = Неопределено,
		КнопкаНазад = Неопределено) Экспорт
	...
	Если ИнтернетПоддержкаПользователейКлиентСервер.ЭтоИнтерфейс85() Тогда
		Если КнопкаНазад <> Неопределено Тогда
			...
			КнопкаНазад.Картинка = БиблиотекаКартинок.ПерейтиНазад;
			КнопкаНазад.Отображение = ОтображениеКнопки.КартинкаИТекст;
		КонецЕсли;
		Если КнопкаДалее <> Неопределено Тогда
			КнопкаДалее.Заголовок = НСтр("ru = 'Далее'");
			КнопкаДалее.Картинка = БиблиотекаКартинок.ПерейтиВперед;
			КнопкаДалее.Отображение = ОтображениеКнопки.КартинкаИТекст;
		КонецЕсли;
	КонецЕсли;
...
// Выполняет чтение архива gzip.
//
// Параметры:
//  РезультатЗагрузки - Структура - результат выполнения операции.
//  ДвоичныеДанные - ДвоичныеДанные - данные архива.
//
// Возвращаемое значение:
//  ДвоичныеДанные - результат чтения.
//
Функция ЧтениеСодержимогоGZip(РезультатЗагрузки, ДвоичныеДанные)
	
	Каталог = ФайловаяСистема.СоздатьВременныйКаталог("gzip");
	
	Попытка
		
		ДанныеВПотоке = ДвоичныеДанные.ОткрытьПотокДляЧтения();
		ЧтениеФайлаАрхива = Новый ЧтениеФайлаАрхива(ДанныеВПотоке, ,ТипФайлаАрхива.GZIP);
