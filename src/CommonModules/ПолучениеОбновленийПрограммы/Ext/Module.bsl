...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Зависимость.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОВерсияхПлатформы;
	Зависимость.ОбращаетсяКВнешнимРесурсам = Истина;
	
	Зависимость = Настройки.Добавить();
	Зависимость.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ПолучениеИУстановкаИсправленийКонфигурации;
	Зависимость.ОбращаетсяКВнешнимРесурсам = Истина;
...
Процедура ИнтернетПоддержкаИСервисы_ПриСозданииНаСервере(Форма) Экспорт
	...
	Элементы.ГруппаОбновлениеПрограммыСтрока2.Видимость     = ПолучениеОбновленийРазрешено;
	...
	Попытка
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				КаталогДистрибутива));
		ЧтениеАрхива = Новый ЧтениеФайлаАрхива(ПутьПолученногоФайла);
		ЧтениеАрхива.ИзвлечьВсе(КаталогДистрибутива, РежимВосстановленияПутейФайлаАрхива.Восстанавливать);
	Исключение
...
Процедура УстановитьОбновлениеПлатформы(Контекст, Параметры, УстановкаОтменена)
	...
	Иначе
		...
		Если Параметры.РежимУстановкиПлатформы = 0 Тогда
			// Часть ошибок появляемые при установки в тихом режиме можно решить в интерактивном режиме.
