...
// См. ПрограммныйИнтерфейсСервиса.ИнформацияОМенеджереСервиса
Функция ИнформацияОМенеджереСервиса() Экспорт
	
	Служебный = ПрограммныйИнтерфейсСервисаСлужебный;
	Метод = "service_manager/info";
	ТипИнтерфейса = "srv";
	
	ДанныеЗапроса = Служебный.ШаблонЗапроса(Метод, ТипИнтерфейса);
	ДанныеЗапроса.Вставить("tenant_url", ПолучитьНавигационнуюСсылкуИнформационнойБазы());
	Адрес = Служебный.АдресИсполненияВнешнегоПрограммногоИнтерфейса(Метод, ТипИнтерфейса);
	Результат = РаботаВМоделиСервисаБТС.ОтправитьЗапросВМенеджерСервиса("POST", Адрес, ДанныеЗапроса);
	
	Если Результат.КодСостояния <> 200 Тогда
		ТекстИсключения = НСтр("ru = 'Не удалось получить информацию о менеджере сервиса, сервис временно недоступен:
			|Ресурс: %1
			|Код ответа: %2
			|Ответ: %3'");
		ВызватьИсключение СтрШаблон(ТекстИсключения, Адрес, Результат.КодСостояния, Результат.ПолучитьТелоКакСтроку());
	КонецЕсли;
	
	ДанныеОтвета = Служебный.РезультатВыполнения(Результат);
	
	Результат = Новый Структура;
	Результат.Вставить("ВерсияМенеджераСервиса", ДанныеОтвета.general.sm_version);
	Результат.Вставить("ЧасовойПоясМенеджераСервиса", ДанныеОтвета.general.sm_timezone);
	Результат.Вставить("ВнешнийАдресМенеджераСервиса", ДанныеОтвета.ext_url);
