...
Процедура УстановитьСертификатВКонтейнерИХранилище(ПараметрыЗаявления, АдресРезультата) Экспорт
	...
	Исключение
		...
		Результат.Вставить("ОписаниеОшибки", ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ВызватьHTTPМетод(HTTPМетод, URL, ПараметрыЗапроса, СоответствиеПолейОтвета, Заголовки = Неопределено, ПараметрыПреобразования = Неопределено)
	...
	Исключение
		...
		Результат.Вставить("Выполнено", Ложь);
		Результат.Вставить("КодОшибки", "ОшибкаПриОбращенииКСерверу");
		Результат.Вставить("ОписаниеОшибки", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		Возврат Результат;
	КонецПопытки;
	...
	Если Ответ.КодСостояния = 200 Тогда
		...
		ContentType = ОбщегоНазначенияБТССлужебныйКлиентСервер.ЗаголовокHTTP(Ответ, "Content-Type");
		...
		ИначеЕсли ContentType = "application/octet-stream" Тогда
			...
			Результат.Вставить("Имя", СтрЗаменить(ОбщегоНазначенияБТССлужебныйКлиентСервер.ЗаголовокHTTP(Ответ, "Content-Disposition"), "attachment; filename=", ""));
		КонецЕсли;
