...
Функция ПодсистемыПоОбъектамМетаданных() Экспорт

	ИменаПодсистем = Новый Соответствие;
	ЗаполнитьИменаПодсистемПоОбъектамМетаданных(ИменаПодсистем);

	Возврат Новый ФиксированноеСоответствие(ИменаПодсистем);
...
Функция КлассыМетаданных()
	...
	Возврат ОбщегоНазначенияБТССлужебныйПовтИсп.КлассыМетаданныхВМоделиКонфигурации();
...
Процедура ЗаполнитьМодельПоФункциональнымОпциям(Знач Модель)
	...
	Для Каждого ФункциональнаяОпция Из Метаданные.ФункциональныеОпции Цикл
		...
		Для Каждого ЭлементСостава Из ФункциональнаяОпция.Состав Цикл
			...
			ОписаниеОбъекта = ОбщегоНазначенияБТССлужебный.СвойстваОбъектаМоделиКонфигурации(
				Модель, ЭлементСостава.Объект); // см. НовыйОписаниеОбъекта
...
Процедура ЗаполнитьМодельПоРазделителям(Знач Модель)
		...
		Если ОбщийРеквизит.РазделениеДанных = Метаданные.СвойстваОбъектов.РазделениеДанныхОбщегоРеквизита.Разделять Тогда
			...
			Для Каждого ЭлементСостава Из ОбщийРеквизит.Состав Цикл
				...
				Если (АвтоИспользованиеОбщегоРеквизита И ЭлементСостава.Использование = АвтоИспользоватьОбщийРеквизит)
					...
					ОписаниеОбъекта = ОбщегоНазначенияБТССлужебный.СвойстваОбъектаМоделиКонфигурации(
						Модель, ЭлементСостава.Метаданные);
	...
	Для Каждого Последовательность Из Метаданные.Последовательности Цикл
		...
		Если Последовательность.Документы.Количество() > 0 Тогда
			...
			ОписаниеПоследовательности = ОбщегоНазначенияБТССлужебный.СвойстваОбъектаМоделиКонфигурации(
				Модель, Последовательность);
			
			Для Каждого Документ Из Последовательность.Документы Цикл
				
				ОписаниеДокумента = ОбщегоНазначенияБТССлужебный.СвойстваОбъектаМоделиКонфигурации(Модель, Документ);
	...
	Для Каждого ЖурналДокументов Из Метаданные.ЖурналыДокументов Цикл
		...
		Если ЖурналДокументов.РегистрируемыеДокументы.Количество() > 0 Тогда
			...
			ОписаниеЖурнала = ОбщегоНазначенияБТССлужебный.СвойстваОбъектаМоделиКонфигурации(Модель, ЖурналДокументов);
	...
	Для Каждого РегистрРасчета Из Метаданные.РегистрыРасчета Цикл
		...
		Если РегистрРасчета.Перерасчеты.Количество() > 0 Тогда
			...
			ОписаниеРегистраРасчета = ОбщегоНазначенияБТССлужебный.СвойстваОбъектаМоделиКонфигурации(
				Модель, РегистрРасчета);
			
			Для Каждого Перерасчет Из РегистрРасчета.Перерасчеты Цикл
				
				ОписаниеПерерасчета = ОбщегоНазначенияБТССлужебный.СвойстваОбъектаМоделиКонфигурации(Модель, Перерасчет);
...
Процедура ЗаполнитьИменаПодсистемПоОбъектамМетаданных(ПодсистемыПоОбъектам, Подсистемы = Неопределено,
	...
	Для Каждого Подсистема Из Подсистемы Цикл
		...
		ИмяРодителяДоРекурсии = ИмяРодителя;

		Если ИмяРодителя = Неопределено Тогда
			ИмяРодителя = Подсистема.Синоним;
		Иначе
			ИмяРодителя = ИмяРодителя + ". " + Подсистема.Синоним;
		КонецЕсли;

		СоставПодсистемы = Подсистема.Состав;
		Для Каждого ОбъектМетаданных Из СоставПодсистемы Цикл

			ПодсистемыОбъекта = ПодсистемыПоОбъектам.Получить(ОбъектМетаданных);
			Если ПодсистемыОбъекта = Неопределено Тогда
				ПодсистемыОбъекта = Новый Массив;
				ПодсистемыПоОбъектам.Вставить(ОбъектМетаданных, ПодсистемыОбъекта);
			КонецЕсли;
			ПодсистемыОбъекта.Добавить(ИмяРодителя);

		КонецЦикла;

		ЗаполнитьИменаПодсистемПоОбъектамМетаданных(ПодсистемыПоОбъектам, Подсистема.Подсистемы, ИмяРодителя);

		ИмяРодителя = ИмяРодителяДоРекурсии;
