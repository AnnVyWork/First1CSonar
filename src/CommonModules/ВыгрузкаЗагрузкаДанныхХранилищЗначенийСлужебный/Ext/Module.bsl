...
Процедура ПриРегистрацииОбработчиковВыгрузкиДанных(ТаблицаОбработчиков) Экспорт
	...
	Для Каждого ЭлементСписка Из СписокМетаданных Цикл
		...
		НовыйОбработчик.Обработчик = ВыгрузкаЗагрузкаДанныхХранилищЗначенийСлужебный;
...
Процедура ПередВыгрузкойНастроек(
	Контейнер,
	Сериализатор,
...
Процедура ПередЗагрузкойНастроек(
	Контейнер,
	ИмяХранилищаНастроек,
	КлючНастроек,
	КлючОбъекта,
...
//	РеквизитыСХранилищемЗначений - Массив из Структура
...
Функция ВыгрузитьХранилищеЗначения(Контейнер, ХранилищеЗначения, Артефакт)
	...
	Иначе
		...
		Попытка
			...
			ОписаниеЗначения = ЗаписатьХранилищеЗначенияВАртефакт(Контейнер, Значение);
			Если ОписаниеЗначения <> Неопределено Тогда
				Артефакт = ОписаниеЗначения;
				Результат = Истина;
			КонецЕсли;
			
		Исключение
			
			Возврат Результат; // Если хранилище не удалось сериализовать - оставим его в объекте
...
Функция ЗаписатьСериализуемоеХранилищеЗначенияВАртефакт(Контейнер, Знач ЗначениеХранилища)
	...
	Если ВЗначенииЕстьОбщиеДанные(Контейнер, ЗначениеХранилища) Тогда
...
// Определяет наличие общих данных, ссылки на которые нужно пересоздать или , в значении хранилища.
//
// Параметры:
//	Контейнер - См. ЗаписатьСериализуемоеХранилищеЗначенияВАртефакт.Контейнер
//	ЗначениеХранилища - См. ЗаписатьСериализуемоеХранилищеЗначенияВАртефакт.ЗначениеХранилища
//
// Возвращаемое значение:
//	Булево - 
//
Функция ВЗначенииЕстьОбщиеДанные(Контейнер, ЗначениеХранилища)
	
	ЕстьОбщиеДанные = Ложь;
	Попытка
		
		ПрефиксПространстваИменТекущейКонфигурации = "cc";
		ПотокXML = Новый ПотокВПамяти;
		
		ЗаписьXML = Новый ЗаписьXML;
		ЗаписьXML.ОткрытьПоток(ПотокXML, "UTF-8");
		...
		ЗаписьXML.ЗаписатьСоответствиеПространстваИмен(
			ПрефиксПространстваИменТекущейКонфигурации, "http://v8.1c.ru/8.1/data/enterprise/current-config");
		
		СериализаторXDTO.ЗаписатьXML(ЗаписьXML, ЗначениеХранилища, НазначениеТипаXML.Явное);
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			ЗначениеАтрибута = ЧтениеXML.ПолучитьАтрибут("type", "http://www.w3.org/2001/XMLSchema-instance");
			Если ЗначениеАтрибута <> Неопределено Тогда
				
				ЧастиТипа = СтрРазделить(ЗначениеАтрибута, ":");
				Если ЧастиТипа.Количество() < 2 Тогда
					Продолжить;
				КонецЕсли;
				
				Если ЧастиТипа[0] = ПрефиксПространстваИменТекущейКонфигурации Тогда
					ТипЗначения = ЧастиТипа[1];
				КонецЕсли;
				
			КонецЕсли;
			
			Если ТипЗначения <> Неопределено Тогда
				ЕстьОбщиеДанные = Контейнер.ДополнительныеСвойства.XMLТипыОбщихДанных.Получить(ТипЗначения) = Истина;
			КонецЕсли;
			
			Если ЕстьОбщиеДанные Тогда
				Прервать;
			КонецЕсли;
	...
	Исключение
		ЕстьОбщиеДанные = Истина;
	КонецПопытки;
	
	Возврат ЕстьОбщиеДанные;
