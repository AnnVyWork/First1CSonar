...
Функция УдалитьФайлыВПопытке(ИмяФайла, ИмяСобытияЖР = Неопределено) Экспорт
	...
	Исключение
		...
		КомментарийСобытияЖР = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ЛогироватьОшибкуИсходящегоЗапросаS3(Соединение, Метод, Запрос, Ответ = Неопределено, ИнформацияОбОшибке = "",
	...
	ИмяСобытияЖР = НСтр("ru = 'Объектное хранилище S3.Исходящий HTTP-запрос'", ОбщегоНазначения.КодОсновногоЯзыка());
