...
Функция ПолучитьРазмерФайла(ПараметрыДоступа, ИзФизическогоХранилища, ИдентификаторХранилища, Идентификатор) Экспорт
	...
	Если Доступ.Размер = Неопределено Тогда
		Если ЗначениеЗаполнено(Доступ.АдресS3) Тогда
			...
			Исключение
				ДанныеОшибки = ДанныеОшибки(Соединение, Метод, Запрос,,
					ПередачаДанныхКлиентСервер.ПодробныйТекстОшибки(ИнформацияОбОшибке()));
				ОшибкаПриВыполненииЗапросаДанных(ДанныеОшибки);
				Возврат Неопределено;
			КонецПопытки;
				
			Если Ответ.КодСостояния <> 206 Тогда
				ДанныеОшибки = ДанныеОшибки(Соединение, Метод, Запрос, Ответ);
				ОшибкаПриПолученииДанных(ДанныеОшибки);
				Возврат Неопределено;
			КонецЕсли;
...
Функция Отправить(ПараметрыДоступа, ВФизическоеХранилище, ИдентификаторХранилища, Данные, Знач ИмяФайла, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		Исключение
			ДанныеОшибки = ДанныеОшибки(Соединение, Метод, ЗапросДанных,,
				ПередачаДанныхКлиентСервер.ПодробныйТекстОшибки(ИнформацияОбОшибке()), Истина);
			ОшибкаПриВыполненииЗапросаДанных(ДанныеОшибки);
			Возврат Неопределено;
		КонецПопытки;
		...
		Иначе
			...
			ДанныеОшибки = ДанныеОшибки(Соединение, Метод, ЗапросДанных, ОтветНаЗапросДанных,, Истина);
			ОшибкаПриОтправкеДанных(ДанныеОшибки);
...
Функция НачатьОтправку(ПараметрыДоступа, ВФизическоеХранилище, ИдентификаторХранилища, Данные, Знач ИмяФайла, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Исключение
		ДанныеОшибки = ДанныеОшибки(Соединение, Метод, ЗапросРесурса,,
			ПередачаДанныхКлиентСервер.ПодробныйТекстОшибки(ИнформацияОбОшибке()));
		ОшибкаПриВыполненииЗапросаДанных(ДанныеОшибки);
		Возврат Неопределено;
	КонецПопытки;
	...
	Если ОтветНаЗапросРесурса.КодСостояния <> 200 Тогда
		ДанныеОшибки = ДанныеОшибки(Соединение, Метод, ЗапросРесурса, ОтветНаЗапросРесурса);
		ОшибкаПриОтправкеДанных(ДанныеОшибки);
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ОшибкаПриПолученииДанных(ДанныеОшибки) Экспорт
	...
	ПередачаДанныхВстраивание.ОшибкаПриПолученииДанных(ДанныеОшибки);
	ПередачаДанныхПереопределяемый.ОшибкаПриПолученииДанных(ДанныеОшибки.Ответ);
	
КонецПроцедуры

Процедура ОшибкаПриОтправкеДанных(ДанныеОшибки) Экспорт
	
	ПередачаДанныхВстраивание.ОшибкаПриОтправкеДанных(ДанныеОшибки);
	ПередачаДанныхПереопределяемый.ОшибкаПриОтправкеДанных(ДанныеОшибки.Ответ);
	
КонецПроцедуры

Процедура ОшибкаПриВыполненииЗапросаДанных(ДанныеОшибки) Экспорт
	
	ПередачаДанныхВстраивание.ОшибкаПриВыполненииЗапросаДанных(ДанныеОшибки);
	ПередачаДанныхПереопределяемый.ОшибкаПриВыполненииЗапросаДанных(ДанныеОшибки);
...
Функция ПолучитьДвоичныеДанныеИзS3(Адрес, Начало = Неопределено, Конец = Неопределено) Экспорт
	...
	Исключение
		ДанныеОшибки = ДанныеОшибки(Соединение, Метод, Запрос,,
			ПередачаДанныхКлиентСервер.ПодробныйТекстОшибки(ИнформацияОбОшибке()));
		ОшибкаПриВыполненииЗапросаДанных(ДанныеОшибки);
		ВызватьИсключение ПередачаДанныхКлиентСервер.КраткийТекстОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ПолучитьS3(АдресS3, Диапазон, ИмяФайла = Неопределено)
	...
	УдалитьВременныйФайл(ИмяФайла);
	ДанныеОшибки = ДанныеОшибки(Соединение, Метод, Запрос, ОтветНаЗапросДанных);
	ОшибкаПриПолученииДанных(ДанныеОшибки);
...
Функция ПолучитьDT(ПараметрыДоступа, Адрес, Куки, Диапазон, ИмяФайла = Неопределено)
	...
	Если ОтветНаЗапросДанных.КодСостояния <> 200 И ОтветНаЗапросДанных.КодСостояния <> 206 Тогда
		ДанныеОшибки = ДанныеОшибки(Соединение, Метод, ЗапросДанных, ОтветНаЗапросДанных);
		ОшибкаПриПолученииДанных(ДанныеОшибки);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ДанныеОшибки(Соединение, Метод, Запрос, Ответ = Неопределено, ИнформацияОбОшибке = "", ОтправленыДвоичныеДанные = Ложь)
	
	ДанныеОшибки = Новый Структура;
	ДанныеОшибки.Вставить("Соединение", Соединение);
	ДанныеОшибки.Вставить("Метод", Метод);
	ДанныеОшибки.Вставить("Запрос", Запрос);
	ДанныеОшибки.Вставить("Ответ", Ответ);
	ДанныеОшибки.Вставить("ИнформацияОбОшибке", ИнформацияОбОшибке);
	ДанныеОшибки.Вставить("ОтправленыДвоичныеДанные", ОтправленыДвоичныеДанные);
