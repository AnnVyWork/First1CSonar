...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Исключение
		...
		Иначе
			...
			СостояниеВыгрузки.Комментарий = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
			СостояниеВыгрузки.Записать();
	...
	Попытка
		...
		Если Результат.КоличествоОбъектов > 0 Или Результат.ЭтоПоследнееСообщение Тогда
			...
			ЗаписьZIP = Новый ЗаписьФайлаАрхива(ИмяФайлаАрхива);
			ЗаписьZIP.Добавить(КаталогВыгрузки + "*", РежимСохраненияПутейФайлаАрхива.СохранятьОтносительныеПути, РежимОбработкиПодкаталоговФайлаАрхива.ОбрабатыватьРекурсивно);
			ЗаписьZIP.Записать();
...
Процедура ЗаданиеЗагрузка() Экспорт
	...
	Файл = Новый Файл(ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(Выборка.ИмяФайла));
	...
	ЧтениеАрхива.ИзвлечьВсе(КаталогЗагрузки, РежимВосстановленияПутейФайлаАрхива.Восстанавливать);
	...
	Исключение
		...
		Иначе 
			...
			СостояниеЗагрузки.ОписаниеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
			СостояниеЗагрузки.Записать();
		КонецЕсли;
	...
	Попытка
		Если Результат.Успешно Тогда
			...
			ФайлыБТССлужебный.УстановитьСрокХраненияФайлаВременногоХранилища(Выборка.ИмяФайла, 60);
	...
	Попытка
		ФайлыБТССлужебный.УдалитьФайлВременногоХранилища(Выборка.ИмяФайла);
	Исключение
		ПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		РегистрацияОшибки(НСтр("ru = 'Загрузка'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеОшибки);
	КонецПопытки;
...
Функция СервисПоддерживаетМиграцию(ИмяСервера, АдресПрограммногоИнтерфейса, АдресРегистрации, АдресВосстановления, РегистрацияРазрешена) Экспорт
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(Шаблон, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке())));
		Возврат Ложь;
	КонецПопытки;
	...
	Если ОписаниеДанных.ИмяФайла = "message" Тогда
		...
		Исключение
			...
			РегистрацияОшибки(НСтр("ru = 'Загрузка сообщения'", ОбщегоНазначения.КодОсновногоЯзыка()), 
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			Результат.Успешно = Ложь;
			Результат.Комментарий = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
...
Процедура ЗагрузитьСообщение(Источник)
	...
	ИмяФайлаВременногоХранилища = ФайлыБТССлужебный.НовыйФайлВременногоХранилища("AppMigrationIn", "zip", 60);
	ИмяФайла = ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(ИмяФайлаВременногоХранилища);
	...
	ЧтениеАрхива = Новый ЧтениеФайлаАрхива(ИмяФайла);
	ЭлементАрхива = ЧтениеАрхива.Элементы.Найти("Manifest.xml");
	...
	ЧтениеАрхива.Извлечь(ЭлементАрхива, ВременныйКаталог, РежимВосстановленияПутейФайлаАрхива.НеВосстанавливать);
	...
	Попытка
		...
		ФайлыБТССлужебный.УстановитьСрокХраненияФайлаВременногоХранилища(ИмяФайлаВременногоХранилища, 60 * 24 * 2); // Двое суток
...
Функция ПолучитьФабрикуСУказаниемТипов()
	...
	Типы = ВыгрузкаЗагрузкаДанныхСобытияСлужебный.ПолучитьТипыТребующиеАннотациюСсылокПриВыгрузке();
...
Функция ЗагрузитьОбщиеДанные(КаталогДанных, ТаблицаДанных) Экспорт
	...
	ОбщиеДанныеСопоставляемыеПоПолямПоиска
...
Процедура ЗарегистрироватьИзменения(Источник)
	...
	Если МиграцияПриложенийПовтИсп.ВыгружаемыеОбъекты().Получить(Источник.Метаданные()) = Неопределено
		Или Не ОбъектРасширенияВходитВСоставПланаОбмена(Источник.Метаданные()) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ДоступноИспользованиеРазделенныхДанных Тогда
		...
		|	И НЕ МиграцияПриложений.Заблокирован";
...
Процедура ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	Попытка
		...
		СообщенияВМоделиСервиса.ОтправитьСообщение(Сообщение, РаботаВМоделиСервиса.КонечнаяТочкаМенеджераСервиса(), Истина);
