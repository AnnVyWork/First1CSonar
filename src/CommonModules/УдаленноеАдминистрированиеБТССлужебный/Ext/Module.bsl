...
Процедура ЗавершитьСеансыОбластиДанных(Знач НомераСеансов, Знач ПарольПользователя, Знач СообщениеПользователям) Экспорт
	...
	КлючТекущийОбластиДанных = РаботаВМоделиСервиса.КлючОбластиДанных();
...
Функция ПараметрыАвторизацииУправляющегоПриложения(Знач ПарольПользователя = Неопределено) Экспорт
	...
	Если ПарольПользователя <> Неопределено Тогда
		...
		Если РаботаВМоделиСервиса.АутентификацияТокеномДоступаВнутреннихВызововПрограммногоИнтерфейса() Тогда
			// Пользователя нужно перечитать. ПользователиИнформационнойБазы.ТекущийПользователь() использовать нельзя,
			// т.к. описание аутентифицированного пользователя возвращается из сохраненных данных в сеансе - изменение
			// данных пользователя без перезапуска приложения при этом игнорируются.
			ПользовательИБ = ПользователиИнформационнойБазы.ТекущийПользователь();
			ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоУникальномуИдентификатору(
				ПользовательИБ.УникальныйИдентификатор);
			Если Не ПользовательИБ.АутентификацияСтандартная
				И РаботаВМоделиСервисаСлужебный.ПарольТекущегоПользователяСовпадаетССохраненным(Пароль) Тогда
...
Функция ИспользуемаяВерсияСервисаУправляющегоПриложения(ПараметрыАвторизации, ТребуемаяВерсия = Неопределено) Экспорт
	...
	ПараметрыПодключения = РаботаВМоделиСервиса.НовыйПараметрыПодключенияКМенеджеруСервиса(
		ПараметрыАвторизации.Адрес, ПараметрыАвторизации.Пользователь, ПараметрыАвторизации.Пароль);
	ПоддерживаемыеВерсии = РаботаВМоделиСервиса.ПолучитьВерсииИнтерфейсаМенеджераСервиса(
		ИмяИнтерфейса, ПараметрыПодключения);
