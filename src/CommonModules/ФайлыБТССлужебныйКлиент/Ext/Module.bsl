...
//
// Параметры:
//   ПараметрыПомещенияФайла - см. ФайлыБТССлужебныйКлиент.ПараметрыПомещенияФайла
Процедура ПоместитьФайлИнтерактивно(ПараметрыПомещенияФайла) Экспорт
	...
	Если ВсеПараметры.СпособПомещенияФайла = "ЗагрузкаВСервис"
		И Не ФайлыБТССлужебныйВызовСервера.ДоступнаЗагрузкаВСервис() Тогда
	...
	Если ВсеПараметры.СпособПомещенияФайла = "ВременноеХранилище" Тогда
		...
		МаксимальныйРазмер = МаксимальныйРазмерВременногоХранилища();
	...
	ТекстПредложения = СтрШаблон(НСтр("ru = 'Для загрузки файлов, размером более %1, требуется установить расширение для работы с 1С:Предприятием.
		...
		ФайлыБТССлужебныйКлиентСервер.ПредставлениеРазмераФайла(ПриемлемыйРазмерВременногоХранилища()));
...
Асинх Функция ПоместитьФайлНаСервер(ИмяФайлаНаКлиенте, ИмяФайлаНаСервере) Экспорт
	...
	Буфер = Новый БуферДвоичныхДанных(ФайлыБТССлужебныйКлиентСервер.РазмерПорцииОбработки(РазмерФайла));
	...
	Пока Позиция < РазмерФайла Цикл
		...
		ФайлыБТССлужебныйВызовСервера.ОтправитьОчереднуюПорцию(
			ИмяФайлаНаСервере,
...
// Логировать ошибку исходящего запроса к S3 и ответ на него в журнал регистрации
// 
// Параметры:
//  Соединение - HTTPСоединение - объект HTTPСоединение для формирования описания события
//  Метод - Строка - HTTP-метод - GET, POST, PUT, DELETE, ...
//  Запрос - HTTPЗапрос - объект HTTPЗапрос для формирования описания события
//  Ответ - HTTPОтвет - объект HTTPОтвет для формирования описания события (не обязательно)
...
//
Процедура ЛогироватьОшибкуИсходящегоЗапросаS3(Соединение, Метод, Запрос, Ответ = Неопределено, ИнформацияОбОшибке = "",
		ОтправленыДвоичныеДанные = Ложь) Экспорт
	
	//@skip-check module-nstr-camelcase
	ИмяСобытияЖР = НСтр("ru = 'Объектное хранилище S3.Исходящий HTTP-запрос'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	...
	ПараметрыЛогирования.СоединениеHTTP = Соединение;
	ПараметрыЛогирования.МетодHTTP = Метод;
	ПараметрыЛогирования.ЗапросHTTP = Запрос;
	ПараметрыЛогирования.ОтветHTTP = Ответ;
	ПараметрыЛогирования.ЛогироватьТелоЗапроса = Ложь;
	ПараметрыЛогирования.ОтправленыДвоичныеДанные = ОтправленыДвоичныеДанные;
	ПараметрыЛогирования.ИнформацияОбОшибке = ИнформацияОбОшибке;
...
// См. ФайлыБТСКлиент.ПараметрыПолученияФайла
...
// См. ФайлыБТСКлиент.ПолучитьФайлИнтерактивно
//
// Параметры:
...
Процедура ПроверкаФайлаПередПомещением(СсылкаНаФайл, Отказ, ПараметрыПроверки)
	...
	Если Не ПараметрыПроверки.РасширениеПодключено Тогда
		МаксРазмерБезРасширения = ПриемлемыйРазмерВременногоХранилища();
		Если СсылкаНаФайл.Размер > МаксРазмерБезРасширения Тогда
			ОбработатьОшибкуПроверки(Отказ, СтрШаблон(НСтр("ru = 'Выбран файл %1 больше %2
				...
				СсылкаНаФайл.Имя, ФайлыБТССлужебныйКлиентСервер.ПредставлениеРазмераФайла(МаксРазмерБезРасширения)));
		КонецЕсли;
	КонецЕсли;
	...
	Если СсылкаНаФайл.Размер > МаксимальныйРазмер Тогда
		Если МаксимальныйРазмер <= 0 Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Размер файла должен быть равен %1'"),
				ФайлыБТССлужебныйКлиентСервер.ПредставлениеРазмераФайла(0));
		Иначе
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Файл %1 слишком большой. Выберите файл менее %2'"), СсылкаНаФайл.Имя,
				ФайлыБТССлужебныйКлиентСервер.ПредставлениеРазмераФайла(МаксимальныйРазмер));
		КонецЕсли;
...
Функция МаксимальныйРазмерВременногоХранилища()
...
Процедура ПослеПолученияРазмера(Размер, ДополнительныеПараметры) Экспорт
	...
	ПараметрыПолучения.Вставить("ПредставлениеФайла", ФайлыБТССлужебныйКлиентСервер.ПредставлениеФайла(
