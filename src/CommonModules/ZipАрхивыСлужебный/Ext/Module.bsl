...
Процедура ДобавитьФайл(Архив, ИмяФайла) Экспорт
	...
	Если Инфо.ЭтоКаталог() Тогда
		ЗаписьАрхива.Добавить(ИмяФайла + ПолучитьРазделительПути() + "*", РежимСохраненияПутейФайлаАрхива.СохранятьОтносительныеПути, РежимОбработкиПодкаталоговФайлаАрхива.ОбрабатыватьРекурсивно);
	Иначе
		ЗаписьАрхива.Добавить(ИмяФайла, РежимСохраненияПутейФайлаАрхива.НеСохранятьПути);
	КонецЕсли;
...
// Параметры:
// 	ДанныеАрхива - см. ПрочитатьАрхив
...
Функция ИзвлечьФайл(ДанныеАрхива, ИмяФайла, Каталог) Экспорт
	...
	ЧтениеАрхива.ИзвлечьВсе(Каталог, РежимВосстановленияПутейФайлаАрхива.Восстанавливать);
...
Функция ПрочитатьАрхивВнутр(Знач Источник, Знач Смещение = 0, Знач Размер = Неопределено)
	...
	Если ТипЗнч(Источник) = Тип("УникальныйИдентификатор") Тогда
		...
		ПараметрыАрхива = ПараметрыАрхиваИзМенеджераСервиса(Источник);
		ПолныйРазмер = ПараметрыАрхива.ПолныйРазмер;
		
		ДанныеАрхива.Вставить("ПараметрыДоступа", ПараметрыАрхива.ПараметрыДоступа);
...
Функция ПараметрыАрхиваИзМенеджераСервиса(ИдентификаторФайла)
	
	ПараметрыАрхива = Новый Структура();
	ПараметрыАрхива.Вставить("ПолныйРазмер", 0);
	ПараметрыАрхива.Вставить("ПараметрыДоступа", Новый Структура());
