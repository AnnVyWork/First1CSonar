...
Функция ОтправитьЗапросВМенеджерСервиса(Метод, Адрес, Знач Данные = Неопределено, КэшСоединения = Истина, Таймаут = 60) Экспорт
	...
	Если КэшСоединения Тогда
		Соединение = РаботаВМоделиСервисаСлужебныйПовтИсп.СоединениеСМенеджеромСервиса(ДанныеСервера, Таймаут);
	Иначе
...
// Устанавливает версию подсистем всех областей данных после обновления версии информационной базы
...
//
Процедура УстановитьВерсиюВсехОбластейДанных(ИдентификаторБиблиотеки, ИсходнаяВерсияИБ, ВерсияМетаданныхИБ) Экспорт
