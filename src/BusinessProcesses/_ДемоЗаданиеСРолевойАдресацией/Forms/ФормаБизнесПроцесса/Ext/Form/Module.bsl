...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ОпределитьИсполнителяИРольИсполнителя();
...
&НаКлиенте
Процедура ПроверяющийРасширеннаяПодсказкаНажатие(Элемент)
	...
	ПараметрыФормы = БизнесПроцессыИЗадачиКлиент.ПараметрыФормыВыбораРолиИсполнителя(Объект.РольПроверяющего,
		Объект.ОсновнойОбъектАдресацииПроверяющий, Объект.ДополнительныйОбъектАдресацииПроверяющий);
...
&НаКлиенте
Процедура ИсполнительРасширеннаяПодсказкаНажатие(Элемент)
...
&НаКлиенте
Процедура ТипПроверяющегоПриИзменении(Элемент)
	
	УстановитьЗаголовокПроверяющего();
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьЗаголовокПроверяющего()
	
	Если ТипЗнч(ТипПроверяющего) = Тип("СправочникСсылка.РолиИсполнителей") Тогда
		Элементы.Проверяющий.Заголовок = НСтр("ru='Роль'");
	Иначе
		Элементы.Проверяющий.Заголовок = НСтр("ru='Пользователь'");
	КонецЕсли;
...
&НаКлиенте
Процедура ОпределитьИсполнителяИРольИсполнителя()
...
Процедура ИнициализироватьФорму()
	...
	Если Объект.Завершен Тогда
		...
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
	...
	ПараметрыСообщения.Сообщение            = ТекстСостояния;
	ПараметрыСообщения.Идентификатор        = "ИнформацияОСостоянии";
	...
	Элементы.Проверяющий.Заголовок = ТипПроверяющего;
...
Процедура ОпределитьОтображениеФормыДляВнешнегоПользователя()
	...
	Элементы.ПроверитьВыполнение.Видимость       = Ложь;
...
Процедура УстановитьСостояниеЭлементов()
	...
	РольИсполнителяСтрокой = РольСтрокой(Объект.ОсновнойОбъектАдресации, Объект.ДополнительныйОбъектАдресации);
	Элементы.Исполнитель.РасширеннаяПодсказка.Заголовок = РольИсполнителяСтрокой;
	...
	Элементы.Проверяющий.РасширеннаяПодсказка.Заголовок = РольПроверяющегоСтрокой;
	Элементы.ГруппаПроверитьИсполнение.Доступность = Объект.ПроверитьВыполнение;
	Элементы.Проверяющий.РасширеннаяПодсказка.Гиперссылка = Объект.ПроверитьВыполнение;
