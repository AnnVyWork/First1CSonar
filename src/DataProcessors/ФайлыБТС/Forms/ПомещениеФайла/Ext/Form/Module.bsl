...
Асинх Процедура ПослеОткрытияПотока(Поток, ДополнительныеПараметры) Экспорт
	...
	Контекст.Вставить("Буфер", Новый БуферДвоичныхДанных(ФайлыБТССлужебныйКлиентСервер.РазмерПорцииОбработки(
...
Функция ПараметрыСоединенияМенеджераСервиса()
	...
	ВнешнийАдресМенеджераСервиса = ПрограммныйИнтерфейсСервиса.ВнешнийАдресМенеджераСервиса();
...
Асинх Функция ОтправитьПорциюВСервис(ДвоичныеДанные, Начало, Конец)
	...
	Если ПараметрыЗагрузки.ТипАдреса = "s3" Тогда
		...
		Исключение
			...
			ФайлыБТССлужебныйКлиент.ЛогироватьОшибкуИсходящегоЗапросаS3(ПараметрыЗагрузки.Соединение, "PUT", Запрос,,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), Истина);
			СообщениеОбОшибке = СтрШаблон(
				НСтр("ru = 'Не удалось отправить часть файла по причине: %1'"), 
				ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			ВызватьИсключение СообщениеОбОшибке;
		КонецПопытки;
		Если Ответ.КодСостояния <> 200 Тогда
			ФайлыБТССлужебныйКлиент.ЛогироватьОшибкуИсходящегоЗапросаS3(ПараметрыЗагрузки.Соединение, "PUT", Запрос, Ответ,,
				Истина);
			СообщениеОбОшибке = СтрШаблон(
		...
		ПараметрыЗагрузки.Части.Добавить(СтрЗаменить(ОбщегоНазначенияБТССлужебныйКлиентСервер.ЗаголовокHTTP(Ответ, "ETag"), """", ""));
...
Процедура ОтменитьЗагрузкуНаСервере()
	...
	СвойстваФайла = ФайлыБТССлужебный.СвойстваФайлаВременногоХранилища(ИмяФайлаИлиАдрес);
	Если СвойстваФайла.Зарегистрирован Тогда
		ФайлыБТССлужебный.УдалитьФайлВременногоХранилища(ИмяФайлаИлиАдрес);
	Иначе
		ИмяФайлаНаСервере = ФайлыБТССлужебный.ПолноеИмяФайлаВСеансе(ИмяФайлаИлиАдрес, ПутьФайлаWindows, ПутьФайлаLinux);
		...
		ФайлыБТССлужебный.УдалитьФайлыВПопытке(ИмяФайлаНаСервере, ИмяСобытияЖР);
	КонецЕсли;
