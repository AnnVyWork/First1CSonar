...
Процедура ВыгрузитьКонстанту()
	...
	ВыгрузкаЗагрузкаУзловПлановОбменовСлужебный.ВыгрузитьИзменения(ТекущийКонтейнер, ТекущийОбъектМетаданных, ТекущийСериализатор);
...
Процедура ВыгрузитьНаборЗаписейПодчиненныйРегистратору()
	...
	ИначеЕсли ОбщегоНазначенияБТС.ЭтоРегистрНакопления(ТекущийОбъектМетаданных)
		...
		Для Каждого Регистраторы Из НаборыРегистраторов Цикл
			...
			ВыборкаПоРегистраторам = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам); //@skip-check query-in-loop
