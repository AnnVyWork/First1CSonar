...
Процедура ОткрытьПрошлыеРезультатыПроверки(Команда)
	...
	РезультатыОтчетаПроверки = РезультатыОтчета(РезультатыОтчета);
	РезультатыОтчетаПроверки.Показать();
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция РезультатыОтчета(ОтчетПроверки)
	
	Поток = Новый ПотокВПамяти();
	ОтчетПроверки.Записать(Поток,ТипФайлаТабличногоДокумента.MXL);
	Поток.Перейти(0, ПозицияВПотоке.Начало);
	МакетПечатнойФормыИзм = Новый ТабличныйДокумент();
	МакетПечатнойФормыИзм.Прочитать(Поток);
	
	Возврат МакетПечатнойФормыИзм;
		...
		Если Проверка.ПроводитьПроверку Тогда
			...
			Исключение
				...
				Если АвтоматическоеТестирование Тогда
					...
					СтруктураОшибки.ОшибочныеДанные.Добавить("Язык.Русский", "Язык.Русский");
...
Процедура ОбновитьГиперссылку()
	...
	Элементы.ОткрытьРезультатыПроверки.Видимость = Истина;
КонецПроцедуры
