...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПодсказкаЗавершениеМиграцииСвернута = Истина;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ЧастьВопроса = "";
	Если ЗначениеЗаполнено(ДатаНачала) И Не ЗначениеЗаполнено(ДатаЗавершения) Тогда
		ЧастьВопроса = НСтр("ru = 'Миграция продолжит выполняться в фоне, для проверки статуса можно снова открыть помощник.'");
	КонецЕсли;
	ТекстВопроса = СтрШаблон("%1
		|%2", ЧастьВопроса, НСтр("ru = 'Закрыть помощник?'"));
...
&НаКлиенте
Процедура ЗакрытьИПродолжитьВФоне(Команда)
...
Процедура ОткрытиеСтраницы(Страница)
	...
	Элементы.ОстановитьМиграцию.Видимость = Ложь;
	Элементы.ЗакрытьИПродолжитьВФоне.Видимость = Ложь;
...
Процедура СтраницаОжиданиеОткрытие(ЭтоПереходНазад)
	...
	Элементы.Далее.Видимость = Не ЗначениеЗаполнено(Задание) И ТребуетсяМонопольныйРежим 
		И Не ЗавершитьМиграциюАвтоматически;
	...
	Элементы.Назад.Видимость = Ложь;
	Элементы.ОстановитьМиграцию.Видимость = Истина;
	Элементы.ОстановитьМиграцию.Доступность = НомерОтправленногоСообщения > 0;
	Элементы.ЗакрытьИПродолжитьВФоне.Видимость = Истина;
...
Процедура ОбновлениеСтатусаПерехода() Экспорт
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, СостояниеПерехода());
	Если Не Элементы.ОстановитьМиграцию.Доступность И НомерОтправленногоСообщения > 0 Тогда
		Элементы.ОстановитьМиграцию.Доступность = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура ДекорацияЗавершениеМиграцииРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	МассивСтрок = Новый Массив;
	Если ПодсказкаЗавершениеМиграцииСвернута Тогда
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
			НСтр("ru = 'При завершении миграции работа в программе будет приостановлена. Срок приостановки зависит от количества внесенных во время миграции изменений.'")));
		МассивСтрок.Добавить(Символы.ПС);
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = ' Свернуть'"),,,,"Свернуть"));
	Иначе
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
			НСтр("ru = 'При завершении миграции работа в программе будет приостановлена.'")));
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = ' Подробнее'"),,,,"Подробнее"));
	КонецЕсли;
	Элементы.ДекорацияЗавершениеМиграцииРасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(МассивСтрок);
	
	ПодсказкаЗавершениеМиграцииСвернута = Не ПодсказкаЗавершениеМиграцииСвернута;
