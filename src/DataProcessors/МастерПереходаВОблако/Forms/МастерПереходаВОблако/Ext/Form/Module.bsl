...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Ширина = 0; 
		Элементы.ПраваПользователейПолноеИмя.ОтображениеКнопкиВыбора = ОтображениеКнопкиВыбора.ОтображатьВВыпадающемСписке;
		Элементы.АдресСервисаСтатус.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Низ;
		Элементы.РегистрацияИмяСтатус.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Низ;
		Элементы.РегистрацияТелефонСтатус.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Низ;
		Элементы.КодАктивацииСтатус.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Низ;
		Элементы.РегистрацияПочтаСтатус.Высота = 2;
	Иначе
		Ширина = 60;
		Элементы.ПраваПользователейПолноеИмя.ОтображениеКнопкиВыбора = ОтображениеКнопкиВыбора.ОтображатьВПолеВвода;
		Элементы.АдресСервисаСтатус.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Центр;
		Элементы.РегистрацияИмяСтатус.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Центр;
		Элементы.РегистрацияТелефонСтатус.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Центр;
		Элементы.КодАктивацииСтатус.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Центр;
		Элементы.РегистрацияПочтаСтатус.Высота = 1;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.РазмерПриложений") Тогда
		МодульРазмерПриложений = ОбщегоНазначения.ОбщийМодуль("РазмерПриложенийСлужебный");
	...
	Элементы.ПарольОткрытый.КартинкаКнопкиВыбора = БиблиотекаКартинок.ПоказатьПароль;
	Элементы.ПарольЗакрытый.КартинкаКнопкиВыбора = БиблиотекаКартинок.СкрытьПароль;
	...
	Иначе
		...
		Элементы.КоличествоЗаданийВыгрузкиДанных.Видимость = Ложь;
	КонецЕсли;
			...
			Если ЗначениеЗаполнено(Строка.Право) И Не ЗначениеЗаполнено(Строка.Идентификатор) Тогда
				ОбщегоНазначения.СообщитьПользователю(
					СтрШаблон(НСтр("ru = 'Для пользователя сервиса ''%1'' с правом ''%2'' не указан пользователь из базы.'"), 
						Строка.ПолноеИмяПользователяИБ, ПредставлениеПраваПользователя(Строка.Право)),, 
					СтрШаблон("ПраваПользователей[%1].ПолноеИмяПользователяИБ", Формат(НомерСтроки,"ЧГ=0")),, Отказ); 
			КонецЕсли; 
			Если ЗначениеЗаполнено(Строка.Идентификатор) И ЗначениеЗаполнено(Строка.ЛогинПользователяСервиса) И Не ЗначениеЗаполнено(Строка.Право) Тогда
				ОбщегоНазначения.СообщитьПользователю(
					СтрШаблон(НСтр("ru = 'Для пользователя сервиса ''%1'' не указано право пользователя из базы.'"), Строка.ПолноеИмяПользователяИБ),, 
...
&НаКлиенте
Процедура ЗарегистрироватьсяОбработкаСсылки(Элемент, НавигационнаяСсылкаСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ВыбратьООПриИзменении(Элемент)
...
Процедура РегистрацияИмяПриИзменении(Элемент)
	...
	ПоказатьПризнакЗаполнения(РегистрацияИмя, "РегистрацияИмяСтатус");
...
Процедура РегистрацияТелефонПриИзменении(Элемент)
	...
	ПоказатьПризнакЗаполнения(РегистрацияТелефон, "РегистрацияТелефонСтатус");
...
Процедура СпособПереходаМиграцияПриИзменении(Элемент)
	...
	Если Не МиграцияДоступна Тогда
		СпособПерехода = 0;
		Возврат; 
	КонецЕсли;
...
&НаКлиенте
Процедура РегистрацияПарольНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	Если Элементы.РегистрацияПароль.РежимПароля Тогда
		Элементы.РегистрацияПароль.КартинкаКнопкиВыбора = БиблиотекаКартинок.СкрытьПароль;
	Иначе
		Элементы.РегистрацияПароль.КартинкаКнопкиВыбора = БиблиотекаКартинок.ПоказатьПароль;
	КонецЕсли; 
...
Процедура ПодтверждениеПриИзменении(Элемент)
	Элементы.Далее.Доступность = Подтверждение;
КонецПроцедуры

&НаКлиенте
Процедура ФильтрСопоставлениеПользователейПриИзменении(Элемент)
	
	Если ФильтрСопоставлениеПользователей = 0 Тогда
		Элементы.ПраваПользователей.ОтборСтрок = Неопределено;
	ИначеЕсли ФильтрСопоставлениеПользователей = 1 Тогда
		Элементы.ПраваПользователей.ОтборСтрок = Новый ФиксированнаяСтруктура("Сопоставлен", Истина);
	ИначеЕсли ФильтрСопоставлениеПользователей = 2 Тогда
		Элементы.ПраваПользователей.ОтборСтрок = Новый ФиксированнаяСтруктура("Сопоставлен", Ложь);
	КонецЕсли;
...
&НаКлиенте
Процедура ПраваПользователейПолноеИмяПриИзменении(Элемент)
	
	ИдентификаторТекущейСтроки = Элементы.ПраваПользователей.ТекущиеДанные.ПолучитьИдентификатор(); 
	СтрокаПраваТекущая = ПраваПользователей.НайтиПоИдентификатору(ИдентификаторТекущейСтроки);
	
	ПользовательСервисаНайден = Ложь;
	Поиск = ПользователиСервиса.НайтиСтроки(Новый Структура("ПолноеИмя", СтрокаПраваТекущая.ПолноеИмяПользователяСервиса));
	Если Поиск.Количество() > 0 Тогда
		ДанныеПользователяСервиса = Поиск[0];    
		ПользовательСервисаНайден = Истина;
	КонецЕсли;
   	
	Поиск = ПраваПользователей.НайтиСтроки(Новый Структура("ПолноеИмяПользователяСервиса", СтрокаПраваТекущая.ПолноеИмяПользователяСервиса));
	Для Каждого СтрокаПрава из Поиск Цикл
		
		СтрокаПрава.Сопоставлен = Ложь;
		
		Если СтрокаПрава = СтрокаПраваТекущая И ПользовательСервисаНайден Тогда
			СтрокаПрава.ПолноеИмяПользователяСервиса = ДанныеПользователяСервиса.ПолноеИмя;
			СтрокаПрава.ЛогинПользователяСервиса = ДанныеПользователяСервиса.Логин;
			СтрокаПрава.ЭлектроннаяПочта = ДанныеПользователяСервиса.ЭлектроннаяПочта;
			СтрокаПрава.ЭтоВладелецАбонента = ДанныеПользователяСервиса.ЭтоВладелецАбонента;
			СтрокаПрава.Право = ?(ДанныеПользователяСервиса.ЭтоВладелецАбонента, ПравоЗапускИАдминистрирование(), ПравоЗапуск());
			СтрокаПрава.Сопоставлен = Истина;
		КонецЕсли;
		
		Если Не СтрокаПрава.Сопоставлен Тогда
			СтрокаПрава.ПолноеИмяПользователяСервиса = "";
			СтрокаПрава.ЛогинПользователяСервиса = ""; 
			СтрокаПрава.ЭлектроннаяПочта = "";
			СтрокаПрава.ЭтоВладелецАбонента = Ложь;
			СтрокаПрава.Право = "";
			
			ПоискПочта = ПользователиИБ.НайтиСтроки(Новый Структура("Идентификатор", СтрокаПрава.Идентификатор));
			Если Поиск.Количество() > 0 Тогда
				СтрокаПрава.ЭлектроннаяПочта = ПоискПочта[0].ЭлектроннаяПочта;
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура ПраваПользователейПолноеИмяНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
Функция ПроверитьВозможностьАвтоматическогоПереходаСРасширениями()	
	...
	Попытка
		...
		ТекстПредупреждения = Вычислить(
			"ОбщегоНазначенияБТССлужебныйВызовСервера.ТекстПредупрежденияОбАктивныхРасширенияхИзменяющихСтруктуруДанных()");
	Исключение
		ТекстПредупреждения = "";
	КонецПопытки;
	...
	Если СпособПерехода = СпособМиграция() И МенеджерСервисаПоддерживаетМиграциюСРасширениями() Тогда
		...
		СтрокиВопроса.Добавить(НСтр("ru = 'Если расширение недоступно для абонента, будет перенесена только каркасная версия этого расширения.'"));
...
Процедура ЗапуститьВыгрузкуДанныхНаСервере()
	...
	Попытка
		...
		Иначе
			ИмяФайлаВременногоХранилища = ФайлыБТССлужебный.НовыйФайлВременногоХранилища("data2xml", "zip", 120);
			ФайлыБТССлужебный.ЗаблокироватьФайлВременногоХранилища(ИмяФайлаВременногоХранилища, УникальныйИдентификатор);
			ИмяФайлаВыгрузки = ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(ИмяФайлаВременногоХранилища);
		...
		ПараметрыВыгрузки.Вставить(
			"ВидЗадания",
			ВыгрузкаЗагрузкаДанныхСлужебный.ВидЗаданияВыгрузкиЗагрузкиДанныхПоИмени("ИнтерактивнаяВыгрузкаДанных"));
...
Асинх Процедура ПроверитьГотовностьВыгрузки() 
	...
	Исключение
		...
		ОбработатьОшибку(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ТекстОшибки = СтрШаблон(НСтр("ru = 'При выгрузке данных произошла ошибка.
			|%1'"),
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		ПоказатьОшибкуНаСтраницеОжидания(ТекстОшибки);
	...
	ПоказыватьПрогрессБар = ПроцентЗавершения <> Неопределено;
	Элементы.ГруппаПроцентЗавершения.Видимость = ПоказыватьПрогрессБар;
	Элементы.СостояниеКартинка.Видимость = Не ПоказыватьПрогрессБар; 
	
	Если ВыполняетсяСохранениеФайла И РасширениеРаботыСФайламиПодключено Тогда
		Для НомерЧасти = 1 По СостояниеВыгрузки.КоличествоЧастей Цикл
			ДвоичныеДанные = ПолучитьЧасть(ИдентификаторСостояния); //@skip-check query-in-loop
...
Процедура СохранитьФайлВыгрузки()
	...
	Если Не ЗначениеЗаполнено(АдресДанныхВыгрузки) Тогда
		Если РазмерФайлаВыгрузки <= ФайлыБТССлужебныйКлиент.ПриемлемыйРазмерВременногоХранилища() Тогда
...
Процедура УдалитьВременныеДанныеПослеСохранения()
	...
	ФайлыБТССлужебный.УдалитьФайлВременногоХранилища(ИмяФайлаВременногоХранилища);
...
Функция СостояниеВыгрузки(ИдентификаторЗадания, ИдентификаторСостояния, АдресХранилища)
	...
	СостояниеВыгрузкиЗагрузки = ВыгрузкаЗагрузкаДанныхСлужебный.СостояниеВыгрузкиЗагрузкиОбластиДанных(
		ИдентификаторСостояния);	
				
	Если ЗначениеЗаполнено(СостояниеВыгрузкиЗагрузки) Тогда
		СостояниеВыгрузки.ПредставлениеСостояния = ВыгрузкаЗагрузкаДанныхСлужебный.ПредставлениеСостоянияВыгрузкиЗагрузкиОбластиДанных(
			СостояниеВыгрузкиЗагрузки);
		СостояниеВыгрузки.ПроцентЗавершения = ВыгрузкаЗагрузкаДанныхСлужебный.ПроцентЗавершенияВыгрузкиЗагрузкиОбластиДанных(
			СостояниеВыгрузкиЗагрузки);
	КонецЕсли;
...
Функция ПереместитьДанныеВыгрузкиИзФайлаВХранилище(ИмяФайлаВременногоХранилища, ИдентификаторФормы)
	...
	ПолноеИмяФайла = ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(ИмяФайлаВременногоХранилища);
...
Процедура ЗапуститьПроверкуВыгружаемыхДанныхНаСервере()
	...
	Попытка
		...
		АдресХранилища = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
		ИмяМетодаПроверкиДанных = "ВыгрузкаЗагрузкаДанныхСлужебный.ПоместитьОшибкиВыгружаемыхДанныхВоВременноеХранилище";
...
Процедура ПереходНазад(ТекущаяСтраница)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаВыгрузкаДанных Тогда
		Если ВыполняетсяСохранениеФайла Тогда
			Если ИмяСтраницыПереходаКВыгрузке = Элементы.СтраницаПриветствие.Имя
				Или ИмяСтраницыПереходаКВыгрузке = Элементы.СтраницаПредупреждениеРасширения.Имя Тогда
				УстановитьСтраницуПриветствие(ТекущаяСтраница);
			Иначе
				УстановитьСтраницуСпособПерехода(ТекущаяСтраница);
			КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПредупреждениеРасширения Тогда
		УстановитьСтраницуВыгрузкиДанных(ТекущаяСтраница, ВыполняетсяСохранениеФайла);
...
Процедура УстановитьСтраницуВводСведений(ТекущаяСтраница)
	...
	ИначеЕсли ВариантРегистрации = ВариантВводКодаАктивации() Тогда
		...
		Элементы.РегистрацияПочта.ОтметкаОбязательногоЗаполнения = Ложь;
		ПроверитьКодАктивации();
	Иначе
		Элементы.ИнформацияВариантаРегистрации.ТекущаяСтраница = Элементы.СтраницаИнформацияПоОрганизации;
		Если ВариантРегистрации = ВариантВыборОО() Тогда
...
Процедура УстановитьСтраницуВыгрузкиДанных(ТекущаяСтраница, ЭтоСохранениеФайла)
	...
	Заголовок = ЗаголовокВыгрузкаДанных();
	Элементы.НаименованиеПриложения.Видимость = Не ВыполняетсяСохранениеФайла;
...
Процедура ПерейтиВФормуМиграцииПриложения()
	...
	Если ПраваПользователей.Количество() > 0 Тогда
		Для Каждого Строка Из ПраваПользователей Цикл
			Если ЗначениеЗаполнено(Строка.ЛогинПользователяСервиса) Тогда
				НоваяСтрока = ПраваПользователейМиграции.Добавить();
				НоваяСтрока.Логин = Строка.ЛогинПользователяСервиса;
				НоваяСтрока.Наименование = Строка.ПолноеИмяПользователяСервиса;
...
Процедура ПроверитьАдресСервиса()
	...
	Если ЗначениеЗаполнено(АдресСервиса) Тогда
		Элементы.Далее.Доступность = Ложь;
		ПоказатьСостояниеОжидание("АдресСервисаСтатус");
...
Процедура Подключаемый_ЗаполнитьНастройки(Результат, ДополнительныеПараметры) Экспорт
	...
	ИмяЭлементаСостояния = "АдресСервисаСтатус";
	...
	Элементы.Восстановить.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Восстановить'"),,,, Данные.url_recover);
	Элементы.Зарегистрироваться.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Зарегистрироваться'"),,,,  СсылкаРегистрация());
...
Процедура ПроверитьАдресПочты()
	...
	ИмяЭлементаСостояния = "РегистрацияПочтаСтатус";
...
Процедура ПроверитьКодАктивации()
	...
	ИмяЭлементаСостояния = "КодАктивацииСтатус";
...
Процедура Подключаемый_ЗаполнитьДанныеПоКодуАктивации(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПоказатьПризнакЗаполнения(РегистрацияИмя, "РегистрацияИмяСтатус");
		ПоказатьСостояниеГотово("РегистрацияПочтаСтатус");
		ПоказатьПризнакЗаполнения(РегистрацияТелефон, "РегистрацияТелефонСтатус");
...
Процедура Подключаемый_ЗаполнитьФормуПослеРегистрации(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось выполнить регистрацию в сервисе'");
	Если Результат.Ошибка Тогда
		ПоказатьОшибкуНаСтраницеОжидания(Результат.СообщениеОбОшибке, ЗаголовокОшибки);
		Возврат;
	КонецЕсли; 
	
	Данные = Результат.Данные;
	Если Данные.error Тогда
		ПоказатьОшибкуНаСтраницеОжидания(Данные.description, ЗаголовокОшибки);
	Иначе
		ПоказатьСостояниеОжидание("СостояниеКартинка");
...
Процедура НачатьАктивациюКодаРегистрации()
	...
	ИмяЭлементаСостояния = "КодАктивацииЗапросНаРегистрациюСтатус";
	Если ЗначениеЗаполнено(КодАктивации) Тогда
		ПоказатьСостояниеОжидание(ИмяЭлементаСостояния);
		ПараметрыЗапроса = ШаблонПараметровЗапросов();
		ПараметрыЗапроса.Вставить("КодАктивации", КодАктивации);
...
Процедура Подключаемый_ЗаполнитьФормуПослеВхода(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Ошибка Тогда
		ПоказатьОшибкуНаСтраницеОжидания(
			Результат.СообщениеОбОшибке, НСтр("ru = 'Вход не выполнен.
				|Произошла ошибка при авторизации в сервисе'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьВыборРасширенияНаСервере(ИмяРасширения, ТекущаяСтрока)
	...
	Если ТекСтрока.Наименование <> СтрокаРасширения.Наименование Тогда
		Если Не ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = ТекстСообщения + Символы.ПС;
		КонецЕсли;
		
		Текст = НСтр("ru = 'Обратите внимание: Наименование выбранного расширения отличается от наименования расширения для восстановления.
			|Возможны ошибки при миграции.'");
		
		ТекстСообщения = ТекстСообщения + Текст;
	КонецЕсли;
...
Процедура Подключаемый_ЗаполнитьСписокПользователей(Результат, ДополнительныеПараметры) Экспорт
	...
	Элементы.ПраваПользователейПолноеИмя.СписокВыбора.Очистить();
	Для Каждого Элемент Из Данные.user Цикл
		НоваяСтрока = ПользователиСервиса.Добавить();
		...
		Элементы.ПраваПользователейПолноеИмя.СписокВыбора.Добавить(Элемент.name, Элемент.name);
	КонецЦикла;
	ПользователиСервиса.Сортировать("ПолноеИмя");
	Элементы.ПраваПользователейПолноеИмя.СписокВыбора.СортироватьПоЗначению();
...
Процедура СоздатьПользователяСервиса()
	...
	Если ТекущиеДанные <> Неопределено И Не ЗначениеЗаполнено(ТекущиеДанные.ПолноеИмяПользователяСервиса) Тогда
...
Процедура СоздатьПользователяСервисаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		НоваяСтрока.ЭлектроннаяПочта = Результат.Почта;
		...
		Элементы.ПраваПользователейПолноеИмя.СписокВыбора.Добавить(Результат.ПолноеИмя);
		Элементы.ПраваПользователейПолноеИмя.СписокВыбора.СортироватьПоЗначению();
		
		Если ЗначениеЗаполнено(Результат.Идентификатор) Тогда
			Поиск = ПраваПользователей.НайтиСтроки(Новый Структура("Идентификатор", Результат.Идентификатор));
			Если Поиск.Количество() > 0 Тогда
				Поиск[0].ПолноеИмяПользователяСервиса = Результат.ПолноеИмя;
				Поиск[0].ЛогинПользователяСервиса = Результат.Логин;
				Поиск[0].ЭлектроннаяПочта = Результат.Почта;
				Поиск[0].ЭтоВладелецАбонента = (Результат.Роль = РольВладелец());
				Поиск[0].Право = ?(Результат.Роль = РольВладелец(), ПравоЗапускИАдминистрирование(), ПравоЗапуск());
				Поиск[0].Сопоставлен = Истина;
				ОбновитьСтатусСопоставленияПользователей();
			КонецЕсли; 
		КонецЕсли;
	КонецЕсли;
...
Процедура СопоставитьПользователей()
	...
	ПраваПользователей.Очистить();
	Для Каждого СтрокаПользовательИБ Из ПользователиИБ Цикл
		Строка = ПраваПользователей.Добавить();
		Строка.Идентификатор = СтрокаПользовательИБ.Идентификатор;
		Строка.Пользователь = СтрокаПользовательИБ.Пользователь;
		Строка.ПолноеИмяПользователяИБ = СтрокаПользовательИБ.ПолноеИмя;
		Строка.ЛогинПользователяИБ = СтрокаПользовательИБ.Логин;
		Строка.ЭлектроннаяПочта = СтрокаПользовательИБ.ЭлектроннаяПочта;
		Строка.Право = "";
	КонецЦикла;
	
	ПоискПрава = ПраваПользователей.НайтиСтроки(Новый Структура("Идентификатор", ИдентификаторТекущегоПользователя));
	ПоискСервис = ПользователиСервиса.НайтиСтроки(Новый Структура("Логин", Логин));
	
	Если ПоискПрава.Количество() > 0 И ПоискСервис.Количество() > 0 Тогда
		ПоискПрава[0].Право = ?(ПоискСервис[0].ЭтоВладелецАбонента, ПравоЗапускИАдминистрирование(), ПравоЗапуск());
		ПоискПрава[0].ЛогинПользователяСервиса = ПоискСервис[0].Логин;
		ПоискПрава[0].ПолноеИмяПользователяСервиса = ПоискСервис[0].ПолноеИмя;
		ПоискПрава[0].Сопоставлен = Истина;
	КонецЕсли; 
	
	Для Каждого СтрокаСервис Из ПользователиСервиса Цикл
		Если ЗначениеЗаполнено(Строка.Идентификатор) Тогда
			Продолжить;
		КонецЕсли; 
		ПоискПрава = ПраваПользователей.НайтиСтроки(Новый Структура("ЛогинПользователяИБ", СтрокаСервис.Логин));
		Если ПоискПрава.Количество() > 0 И Не ЗначениеЗаполнено(ПоискПрава[0].ЛогинПользователяСервиса) Тогда
			ПоискПрава[0].Право = ?(СтрокаСервис.ЭтоВладелецАбонента, ПравоЗапускИАдминистрирование(), ПравоЗапуск());
			ПоискПрава[0].ЛогинПользователяСервиса = СтрокаСервис.Логин;  
			ПоискПрава[0].ПолноеИмяПользователяСервиса = СтрокаСервис.ПолноеИмя;
			ПоискПрава[0].Сопоставлен = Истина;
		КонецЕсли;
		ПоискПрава = ПраваПользователей.НайтиСтроки(Новый Структура("ПолноеИмяПользователяИБ", СтрокаСервис.ПолноеИмя));
		Если ПоискПрава.Количество() > 0 И Не ЗначениеЗаполнено(ПоискПрава[0].ЛогинПользователяСервиса) Тогда
			ПоискПрава[0].Право = ?(СтрокаСервис.ЭтоВладелецАбонента, ПравоЗапускИАдминистрирование(), ПравоЗапуск());
			ПоискПрава[0].ЛогинПользователяСервиса = СтрокаСервис.Логин;
			ПоискПрава[0].ПолноеИмяПользователяСервиса = СтрокаСервис.ПолноеИмя;  
...
Процедура СоздатьПриложениеИзФайла()
	...
	Для Каждого Строка Из ПраваПользователей Цикл
		Если ЗначениеЗаполнено(Строка.Право) Тогда
			...
			ПользовательПриложения.Вставить("login", Строка.ЛогинПользователяСервиса);
...
Функция ПроверитьРезультатЗапросаНаСервере(ИдентификаторЗадания, АдресХранилища)
	...
	ИначеЕсли ФЗ <> Неопределено И ФЗ.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ФЗ.ИнформацияОбОшибке);
...
Процедура ПриИзмененииСпособаПереходаНаГлавной()
	...
	ИначеЕсли СпособПерехода = СпособФайл() Тогда
		...
		ПоказатьСостояниеПусто("АдресСервисаСтатус");
		Элементы.Далее.Доступность = Истина;
	КонецЕсли; 
...
&НаКлиенте
Процедура ПоказатьОшибкуНаСтраницеОжидания(СообщениеОбОшибке, Заголовок = "")
	
	ТекстОшибки = СообщениеОбОшибке;
	Если ЗначениеЗаполнено(Заголовок) Тогда
		Элементы.ОшибкаРекомендация.Заголовок = Заголовок;
	КонецЕсли;
...
Процедура ПоказатьСостояниеОжидание(ИмяЭлемента)
	...
	ЭлементФормы = Элементы[ИмяЭлемента];
	Если ТипЗнч(ЭлементФормы) = Тип("ДекорацияФормы") Тогда
		ЭлементФормы.Картинка = БиблиотекаКартинок.ДлительнаяОперация48;
	ИначеЕсли ТипЗнч(ЭлементФормы) = Тип("ПолеФормы") Тогда
		ЭлементФормы.КнопкаВыбора = Истина;
		ЭлементФормы.КартинкаКнопкиВыбора = БиблиотекаКартинок.ДлительнаяОперация48;
	КонецЕсли;
...
Процедура ПоказатьСостояниеОшибка(ИмяЭлемента)
	...
	ПоказатьСостояниеПусто(ИмяЭлемента);
	
	//ЭлементФормы = Элементы[ИмяЭлемента];
	//Если ТипЗнч(ЭлементФормы) = Тип("ДекорацияФормы") Тогда
	//	ЭлементФормы.Картинка = БиблиотекаКартинок.ДиалогСтоп;
	//ИначеЕсли ТипЗнч(ЭлементФормы) = Тип("ПолеФормы") Тогда
	//	ЭлементФормы.КнопкаВыбора = Истина;
	//	ЭлементФормы.КартинкаКнопкиВыбора = БиблиотекаКартинок.ОформлениеЗнакКрест;
	//КонецЕсли;
...
Процедура ПоказатьСостояниеГотово(ИмяЭлемента)
	...
	ЭлементФормы = Элементы[ИмяЭлемента];
	Если ТипЗнч(ЭлементФормы) = Тип("ДекорацияФормы") Тогда
		ЭлементФормы.Картинка = БиблиотекаКартинок.Успешно;
	ИначеЕсли ТипЗнч(ЭлементФормы) = Тип("ПолеФормы") Тогда
		ЭлементФормы.КнопкаВыбора = Истина;
		ЭлементФормы.КартинкаКнопкиВыбора = БиблиотекаКартинок.Успешно;
	КонецЕсли;
...
Процедура ПриИзмененииВариантаРегистрации()
	...
	Элементы.ОбслуживающаяОрганизация.Доступность = ВариантРегистрации = ВариантВыборОО();
...
Процедура УстановитьВидимостьПоСостояниюОбновления()
	...
	ОбновлятьПоСсылке = ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы");
	
	Элементы.СпособПереходаМиграция.Доступность = МиграцияДоступна;
	
	Элементы.СпособПереходаВыгрузка.Доступность = ВыгрузкаДоступна;
	
	Если Не ВыгрузкаДоступна Тогда
		СоставЗаголовка = Новый Массив;
		СоставЗаголовка.Добавить(Элементы.ПолучитьФайлВыгрузки.РасширеннаяПодсказка.Заголовок);
	
		СоставЗаголовка.Добавить(НСтр("ru = '
			|Для перехода нужно'"));
		СоставЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = ' обновить'"),
				,,,
				?(ОбновлятьПоСсылке, "Обновить", ""))); 
				
		СоставЗаголовка.Добавить(СтрШаблон(
			НСтр("ru = ' приложение до версии %1'"), МинимальнаяВерсияДляВыгрузки));  
			
		Элементы.ПолучитьФайлВыгрузки.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(СоставЗаголовка);
	КонецЕсли;
		
	Если Не МиграцияДоступна Тогда   
		СоставЗаголовка = Новый Массив;
		СоставЗаголовка.Добавить(Элементы.МиграцияШапка.РасширеннаяПодсказка.Заголовок);
		...
		ПоддерживаемыеВерсии = ПоддерживаемыеВерсииДляМиграции.ВыгрузитьЗначения();
		Если ПоддерживаемыеВерсии.Количество() = 1 Тогда
			СоставЗаголовка.Добавить(СтрШаблон(
				НСтр("ru = ' приложение до версии %1'"),ПоддерживаемыеВерсии[0]));
		ИначеЕсли ПоддерживаемыеВерсии.Количество() > 1 Тогда
			СоставЗаголовка.Добавить(СтрШаблон(
				НСтр("ru = ' приложение до одной из версий: %1'"), 
				СтрСоединить(ПоддерживаемыеВерсии, "; ")));
		КонецЕсли;
			
		Элементы.МиграцияШапка.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(СоставЗаголовка);
	КонецЕсли;
...
Функция ПолучитьЧасть(ИдентификаторСостояния)
	...
	ИмяФайла = ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(Выборка.ИмяФайлаВременногоХранилища);
	...
	Запись.Удалить();
	ФайлыБТССлужебный.УдалитьФайлВременногоХранилища(Выборка.ИмяФайлаВременногоХранилища);
...
Функция РазмерПриложенияБольшеДопустимого()
	...
	Возврат ВыгрузкаЗагрузкаДанныхСлужебный.РазмерПриложенияБольшеДопустимого();
