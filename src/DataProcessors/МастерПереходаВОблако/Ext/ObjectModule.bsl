...
Процедура ПередатьФайл(Параметры, АдресРезультата) Экспорт
	...
	РасположениеФайла = ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(Параметры.ИмяФайлаВременногоХранилища);
	...
	Если Результат.Данные.type = "s3" Тогда
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ПоместитьВоВременноеХранилище(Результат, АдресРезультата);				
				ФайлыБТССлужебный.ЛогироватьОшибкуИсходящегоЗапросаS3(Соединение, "PUT", ЗапросДанных,,
			...
			Если Ответ.КодСостояния <> 200 Тогда
				...
				ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
				ФайлыБТССлужебный.ЛогироватьОшибкуИсходящегоЗапросаS3(Соединение, "PUT", ЗапросДанных, Ответ,, Истина);
				Возврат;
			КонецЕсли;
			Части.Добавить(СтрЗаменить(ОбщегоНазначенияБТССлужебныйКлиентСервер.ЗаголовокHTTP(Ответ, "ETag"),"""",""));
