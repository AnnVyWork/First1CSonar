...
//
// Параметры:
//	ПараметрыВыгрузки - см. ВыгрузкаЗагрузкаДанныхСлужебный.ВыгрузитьДанныеТекущейОбластиВАрхив.ПараметрыВыгрузки
//
Процедура ИнициализироватьВыгрузку(Знач ПараметрыВыгрузки) Экспорт
	...
	ВременныйКаталог = ПолучитьИмяВременногоФайла("zip") + ПолучитьРазделительПути(); //@skip-check missing-temporary-file-deletion
	...
	Если Не ЗначениеЗаполнено(ИмяФайлаВыгрузки) Тогда
		ИмяФайлаВыгрузки = ПолучитьИмяВременногоФайла("zip"); //@skip-check missing-temporary-file-deletion
	КонецЕсли;
	
	Архив = ZipАрхивыСлужебный.Создать(ИмяФайлаВыгрузки);
...
Функция ФинализироватьВыгрузку() Экспорт
	...
	ZipАрхивыСлужебный.Завершить(Архив);
	
	Если ВыгрузитьВТом Тогда
		
		Возврат ВыгрузкаЗагрузкаДанныхСлужебный.ВыгрузитьАрхивДанныхВТом(Архив.ИмяФайла, СоставнаяЗагрузка);
	...
	ИначеЕсли ВыгрузитьНаКлиента Тогда
		...
		ИмяФайлаВременногоХранилища = ФайлыБТССлужебный.НовыйФайлВременногоХранилища(Префикс, "zip", 120);
...
//
// Параметры:
//	ДанныеФайла - Строка, УникальныйИдентификатор, Структура - имя файла, идентификатор файла или данные файла полученные с помощью ZipАрхивыСлужебный.ПрочитатьАрхив().
...
Процедура ИнициализироватьЗагрузку(Знач ДанныеФайла, Знач ПараметрыЗагрузки) Экспорт
	...
	ВременныйКаталог = ПолучитьИмяВременногоФайла("zip") + РазделительПути; //@skip-check missing-temporary-file-deletion
	СоздатьКаталог(ВременныйКаталог);
...
Функция ВидЗадания() Экспорт
	...
	ИмяВидаЗадания = ?(ДляЗагрузки, "ПрочаяЗагрузкаДанных", "ПрочаяВыгрузкаДанных");
	
	Возврат ВыгрузкаЗагрузкаДанныхСлужебный.ВидЗаданияВыгрузкиЗагрузкиДанныхПоИмени(ИмяВидаЗадания);
...
Процедура ДобавитьФайлВАрхив(КаталогАрхива)
	...
	ZipАрхивыСлужебный.ДобавитьФайл(Архив, КаталогАрхива);
	УдалитьФайлы(КаталогАрхива);
	
	Если ВыгрузитьВТом И ZipАрхивыСлужебный.Размер(Архив) > 1024 * 1024 * 1024 Тогда
		
		ВыгрузкаЗагрузкаДанныхСлужебный.ДобавитьФайлВАрхивНаТоме(Архив, СоставнаяЗагрузка);
		
	ИначеЕсли ВыгрузитьНаКлиента И ZipАрхивыСлужебный.Размер(Архив) > 10 * 1024 * 1024 Тогда 
		
		ВыгруженоНаКлиентаЧастей = ВыгруженоНаКлиентаЧастей + 1;
		Префикс = СтрШаблон("data2xml-%1", Формат(ВыгруженоНаКлиентаЧастей, "ЧГ=0"));
		ИмяФайлаВременногоХранилища = ФайлыБТССлужебный.НовыйФайлВременногоХранилища(Префикс, "zip", 120);
		ИмяФайлаВыгрузки = ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(ИмяФайлаВременногоХранилища);
		ZipАрхивыСлужебный.ОтделитьЧасть(Архив, ИмяФайлаВыгрузки);
		ЗарегистрироватьЧастьВыгрузки(ВыгруженоНаКлиентаЧастей, ИмяФайлаВременногоХранилища);
...
Процедура ЗаписатьДайджест(ИмяФайла)
	...
	ПотокЗаписи.ЗаписатьТекст(XMLСтрока(ВыгрузкаЗагрузкаУзловПлановОбменовСлужебный.ВыгружатьЗарегистрированныеИзмененияДляУзловПланаОбмена(ЭтотОбъект)));
	ПотокЗаписи.ЗаписатьКонецЭлемента();
...
Процедура ЗаписатьИнформациюОРасширениях()
	...
	Если РазделениеВключено Тогда
		ПоставляемыеРасширения = РасширенияВМоделиСервисаСлужебный.РасширенияТекущейОбластиДанных();
	КонецЕсли;
	...
	Для Каждого РасширениеКонфигурации Из АктивныеРасширения Цикл
		...
		Если ДанныхПоставляемогоРасширенияДостаточноДляРезервногоКопирования(ЗаписьПоставляемогоРасширения) Тогда
			...
			Если ИзменяетСтруктуруДанных Тогда
				...
				ДанныеКаркаса = РасширенияВМоделиСервисаСлужебный.ПолучитьКаркасДляВерсииРасширения(ЗаписьПоставляемогоРасширения);
...
Процедура ЗаписатьИнформациюОВидахПриложения()
	...
	Индекс = 0;
	ИмяВидаПриложенияПользовательское = ВидыПриложенийСлужебный.ИмяТекущегоПользовательскогоВидаПриложения();
...
Функция ИменаРасширенийВидовПриложений()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ВидыПриложений") Тогда 
		...
		МодульВидыПриложенийСлужебный.ИменаРасширенийВидовПриложений(Результат);
	КонецЕсли;
...
Процедура ИнициализироватьДополнительныеСвойстваВыгрузки()
	...
	XMLТипыОбщихДанных = Новый Соответствие;
	Для Каждого ОбъектМетаданных Из ДополнительныеСвойства.ОбщиеДанныеТребующиеСопоставленияСсылок Цикл
		XMLТипыОбщихДанных.Вставить(ВыгрузкаЗагрузкаДанныхСлужебный.XMLТипСсылки(ОбъектМетаданных), Истина);
	КонецЦикла;
	
	Для Каждого Тип Из МиграцияПриложенийПовтИсп.ТипыПересоздаваемыхСсылок() Цикл
		XMLТипыОбщихДанных.Вставить(ВыгрузкаЗагрузкаДанныхСлужебный.XMLТипСсылки(Новый (Тип.Ключ)), Истина);
	КонецЦикла;
	...
	Для Каждого Тип Из МиграцияПриложений.ТипыУзловОбмена() Цикл
		XMLТипыОбщихДанных.Вставить(ВыгрузкаЗагрузкаДанныхСлужебный.XMLТипСсылки(Новый (Тип.Ключ)), Истина);
	КонецЦикла;
	
	ДополнительныеСвойства.Вставить("XMLТипыОбщихДанных", XMLТипыОбщихДанных);
...
Процедура ЗаполнитьКоличествоОбрабатываемыхОбъектов()
	...
	Если Не ВыгрузкаЗагрузкаДанныхСлужебный.НеобходимоПодсчитыватьКоличествоОбъектов(Параметры) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИнициализироватьСостояние()
	...
	ФиксироватьСостояние = ВыгрузкаЗагрузкаДанныхСлужебный.НеобходимоФиксироватьСостояниеВыгрузкиЗагрузкиОбластиДанных(
		Параметры); 
