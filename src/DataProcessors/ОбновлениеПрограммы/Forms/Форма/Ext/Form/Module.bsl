...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Интерфейс85
	Если ИнтернетПоддержкаПользователейКлиентСервер.ЭтоИнтерфейс85() Тогда
		Элементы.КнопкаОтмена.Видимость = Ложь;
	Иначе
		ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиФормы.Низ;
		Элементы.УстановитьОбновлениеИзФайла.Видимость = Ложь;
		...
		Элементы.Декорация17.Видимость = Истина;
		Элементы.Декорация19.Видимость = Истина;
	КонецЕсли;
	// Конец ИнтерфейсТакси
	...
	// Используется для исключения эффекта "мерцания" при управлении отображением элементов.
	...
	Если ЭтоПодчиненныйУзелРИБ
		...
		// В подчиненном узле РИБ обновление программы выполняется через механизмы распределенной информационной базы
		СообщениеЖурнала =
	...
	Если ЭтоФайловаяИБ Тогда
		Если Не ПолучениеОбновленийПрограммы.ЭтоБазоваяВерсияКонфигурации() Тогда
			ПараметрыПолученияОбновлений = ПолучениеОбновленийПрограммы.СлужебнаяПараметрыПолученияОбновлений();
			ДоступноСохранениеДистрибутиваПлатформы =
				ПараметрыПолученияОбновлений.ВыбиратьКаталогСохраненияДистрибутиваПлатформы;
		КонецЕсли;
	Иначе
		ДоступноСохранениеДистрибутиваПлатформы = Истина;
	КонецЕсли;
	
	Элементы.ГруппаКаталогСохраненияПлатформы.Видимость = ДоступноСохранениеДистрибутиваПлатформы;
	...
	Если ЭтоСценарийРабочегоОбновления(ЭтотОбъект)
		...
		Иначе
			...
			Иначе
				...
				// ИнтерфейсТакси
				Если Не ИнтернетПоддержкаПользователейКлиентСервер.ЭтоИнтерфейс85() Тогда
				...
				Иначе
					Элементы.УстановитьОбновлениеИзФайла.Видимость = Истина;
				КонецЕсли;
				// Конец ИнтерфейсТакси
...
&НаКлиенте
Процедура Подключаемый_НадписьОтложенныеОбработчикиОбработкаНавигационнойСсылки(
	Элемент,
	НавигационнаяСсылкаФорматированнойСтроки,
	СтандартнаяОбработка)
...
Процедура ДекорацияУстановитьОбновлениеИзФайла1Нажатие(Элемент)
	...
	УстановитьОбновлениеИзФайлаНаКлиенте();
...
&НаКлиенте
Процедура ДекорацияПатчиВерсияКонфигурацииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура НадписьВерсияУстановленоОбработкаНавигационнойСсылки(
	Элемент,
	НавигационнаяСсылкаФорматированнойСтроки,
	СтандартнаяОбработка)
...
&НаКлиенте
Процедура Подключаемый_АктивныеПользователи(Команда)
...
&НаКлиенте
Процедура УстановитьОбновлениеИзФайла(Команда)
...
&НаКлиенте
Процедура ПараметрыУстановкиПлатформы(Команда)
	
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("РежимУстановкиПлатформы"                , РежимУстановкиПлатформы);
	ПараметрыФормы.Вставить("СохранятьДистрибутивыПлатформыВКаталог" , СохранятьДистрибутивыПлатформыВКаталог);
	ПараметрыФормы.Вставить("КаталогСохраненияПлатформы"             , КаталогСохраненияПлатформы);
	ПараметрыФормы.Вставить("ДоступноСохранениеДистрибутиваПлатформы", ДоступноСохранениеДистрибутиваПлатформы);
	
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПараметрыУстановкиПлатформыЗавершение", ЭтотОбъект);
	
	ОткрытьФорму(
		"Обработка.ОбновлениеПрограммы.Форма.ПараметрыУстановкиПлатформы",
		ПараметрыФормы,
		ЭтотОбъект,
		ЭтотОбъект,
		,
...
// Используется для исключения одновременной видимости нескольких страниц.
//
...
Процедура ОбработатьНавигационнуюСсылку(Ссылка)
	...
	Если Ссылка = "open:V8Update" Тогда
...
&НаКлиенте
Процедура УстановитьОбновлениеИзФайлаНаКлиенте()
...
Процедура ВыполнитьОбработкуКомандыДалее()
	...
	ИначеЕсли ТекСтраница = Элементы.ВыборРежимаОбновленияФайловыйРежим
		...
		// Вызов программного интерфейса СтандартныеПодсистемы.ОбновлениеКонфигурации для установки обновления.
...
Процедура ОтобразитьИнформациюОДоступномОбновленииПлатформы(ЗаполнитьНачальныеНастройки = Ложь)
	...
	Иначе
		...
		Иначе
			...
			Элементы.НадписьВерсия.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = '<b>Версия %1</b>
					|Размер дистрибутива: <b>%2</b>'"),
				ОбновлениеПлатформы.Версия,
			...
			Элементы.КнопкаНазад.Видимость    = ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект);
			Элементы.КнопкаОтмена.Доступность = Истина;
			Элементы.ПродолжитьРаботуНаТекущейВерсии.Видимость = Ложь;
...
Процедура ОтобразитьСтраницуНоваяВерсияПлатформыУжеУстановленаНаКомпьютере()
	...
	Если Не ПустаяСтрока(ОбновлениеПлатформы.URLОсобенностиПерехода) Тогда
		...
		Элементы.НадписьВерсияУстановлено.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Версия %1</b>
				|Версия платформы %1 уже установлена на компьютере.
	...
	Элементы.КнопкаНазад.Видимость    = Не ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект);
	Элементы.КнопкаДалее.Доступность  = Истина;
	Элементы.КнопкаОтмена.Доступность = Истина;
	Элементы.ПродолжитьРаботуНаТекущейВерсии.Видимость = Ложь;
	Элементы.КнопкаДалее.Заголовок    = НСтр("ru = 'Готово'");
...
Процедура ОбновитьПанельПредупреждений(ДоступноОбновление)
	...
	Элементы.НадписьЕстьОшибкиУчета.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = '%1. В программе выявлены <a href=""Перейти к отчету по проблемам"">проблемы ведения учета</a>, которые рекомендуется устранить до обновления.'"),
		НомерОшибки);
	НомерОшибки = НомерОшибки + Элементы.НадписьЕстьОшибкиУчета.Видимость;
	...
	Элементы.НадписьЕстьРасширения.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = '%1. Имеются установленные <a href=""e1cib/app/ОбщаяФорма.Расширения"">расширения</a>. Проверьте совместимость расширений с новой версией конфигурации.'"),
		НомерОшибки);
	НомерОшибки = НомерОшибки + Элементы.НадписьЕстьРасширения.Видимость;
	
	Элементы.НадписьЕстьНеПримененныеИсправления.Видимость = ЕстьНеПримененныеИсправления;
	Элементы.НадписьЕстьНеПримененныеИсправления.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = '%1. Найдены не примененные исправления (патчи). Перед обновлением рекомендуется выполнить <a href=""action:reboot"">перезапуск программы</a>.'"),
		НомерОшибки);
...
Процедура ДобавитьЭлементыВариантаОбновления(ИнформацияОКонфигурации, ИдентификаторВерсии)
	...
	ГруппаВарианта.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	...
	Если ИдентификаторВерсии = ИдентификаторТекущейВерсии()
		...
		Если ЕстьНоваяКонфигурация Тогда
			Если ПустаяСтрока(ИнформацияОКонфигурации.URLНовоеВВерсии) Тогда
				ДополнительнаяИнформацияОВерсии = " " + НСтр("ru = 'Доступно обновление'");
			Иначе
				ДополнительнаяИнформацияОВерсии = СтроковыеФункции.ФорматированнаяСтрока(
					" " + НСтр("ru = 'Доступно обновление. <a href=""%1"">Новое в версии</a>'"),
	...
	ИначеЕсли Не ПустаяСтрока(ИнформацияОКонфигурации.URLНовоеВВерсии) Тогда
		ДополнительнаяИнформацияОВерсии = СтроковыеФункции.ФорматированнаяСтрока(
			" " + НСтр("ru = '<a href=""%1"">Новое в версии</a>'"),
			ИнформацияОКонфигурации.URLНовоеВВерсии);
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьНачальныеНастройкиИОтображениеУстановкиКомпонентов()
	...
	Если ОбновитьПлатформу
		...
		И ДоступноСохранениеДистрибутиваПлатформы Тогда
		НастройкиОбновления = ПолучениеОбновленийПрограммы.НастройкиАвтоматическогоОбновления();
		РежимУстановкиПлатформы = НастройкиОбновления.РежимУстановки;
		Если ДоступноСохранениеДистрибутиваПлатформы Тогда
			Если НастройкиОбновления.КаталогДистрибутивовПлатформы = Неопределено Тогда
				КаталогСохраненияПлатформы = КаталогДистрибутиваПлатформыПоУмолчанию();
				СохранятьДистрибутивыПлатформыВКаталог = Ложь;
			Иначе
	...
	Иначе
		...
		Если ПрямоеОбновление Тогда
			Если ПустаяСтрока(ТекстСообщения) Тогда
				ТекстСообщения = НСтр("ru = 'Доступно обновление программы.'");
			КонецЕсли;
		Иначе
			ТекстСообщения = ТекстСообщения
...
Процедура НастроитьКнопкиКоманднойПанелиДляКомпонентов()
	...
	ИначеЕсли Не ИнформацияОДоступномОбновлении.ДоступноОбновление Или Не ЭтоАдминистраторСистемы Тогда
		...
		Элементы.КнопкаНазад.Видимость  = ДоступноНесколькоВерсийДляОбновлений();
		Элементы.КнопкаДалее.Видимость  = Ложь;
		Элементы.КнопкаОтмена.Заголовок = НСтр("ru = 'Закрыть'");
	...
	Если КоличествоИсправленийДляВерсии > 0 Тогда
		...
		Если ОбновитьКонфигурацию Тогда
			...
			Элементы.ДекорацияПатчиВерсияКонфигурации.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = '<a href=""ВыбратьИсправления"">Исправляемые ошибки (выбрано %1 из %2)</a> для новой версии конфигурации <b>%3</b>'"),
		...
		Иначе
			...
			Элементы.ДекорацияПатчиВерсияКонфигурации.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = '<a href=""ВыбратьИсправления"">Исправляемые ошибки (выбрано %1 из %2)</a> для текущей версии конфигурации <b>%3</b>'"),
				СписокВыбранныхИсправлений.Количество(),
				КоличествоИсправленийДляВерсии,
				ИнтернетПоддержкаПользователей.ВерсияКонфигурации());
...
Процедура ОтобразитьКомпонентПлатформы()
	...
	ИначеЕсли Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Элементы.ПараметрыУстановкиПлатформы.Видимость    = Ложь;
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Элементы.ПараметрыУстановкиПлатформы.Видимость             = ЭтоАдминистраторСистемы;
			Элементы.ДекорацияПояснениеПлатформаКлиентСервер.Видимость = Ложь;
		...
		Иначе
			...
			Элементы.ОбновитьПлатформу.Видимость                       = Ложь;
...
Процедура ИтерацияПолученияФайловОбновления()
	...
	// Если не произошел переход на новую итерацию загрузки файлов, тогда все файлы успешно получены.
	Если ОбновитьКонфигурацию Тогда
	...
	Иначе
		...
		// Если устанавливаются только исправления, тогда установить исправления.
...
Процедура ИнициализироватьСтраницуПолучениеОбновления()
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выполняется получение файла дистрибутива платформы 1С:Предприятие (%1). Получение файла может занять от нескольких минут до нескольких часов в зависимости от размера обновления, скорости подключения к Интернету и производительности компьютера.'"),
				РазмерОбновленияСтр(ЭтотОбъект));
	Иначе
		// Другие сценарии.
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выполняется получение файлов обновления (%1). Получение файлов может занять от нескольких минут до нескольких часов в зависимости от размера обновления, скорости подключения к Интернету и производительности компьютера.'"),
				РазмерОбновленияСтр(ЭтотОбъект));
	КонецЕсли;
...
Процедура ОтобразитьСообщениеСервиса(Сообщение, ЗаголовокКнопкиДалее, ПодробноеОписаниеВнутрОшибки = "")
	...
	Элементы.КнопкаНазад.Видимость =
...
Процедура ПослеПолученияПараметровАдминистрирования(Результат, Инициализировать) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Не удалось установить обновление программы, т.к. не были введены корректные параметры администрирования информационной базы.'");
		ПолучениеОбновленийПрограммыКлиент.ЗаписатьОшибкуВЖурналРегистрации(ТекстСообщения);
		
		ОтобразитьВнутреннююОшибку(
			НСтр("ru = 'Не удалось установить обновление программы, т.к. не были введены корректные параметры администрирования информационной базы.'"),
				,
				ТекстСообщения);
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Если Инициализировать Тогда
				...
				Если ПустаяСтрока(ВыбранноеОбновление.Конфигурация.URLПорядокОбновления) Тогда
					ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(
						ЭтотОбъект,
						Элементы.ПорядокОбновленияФайл.Имя);
				Иначе
					Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
					...
					ВставитьВажноеСообщение(
						Элементы.ПорядокОбновленияФайл,
						"",
						Сообщение,
						"Предупреждение",
						Элементы.ПорядокОбновленияФайл.Имя);
			...
			Если СоединенияИнфо.НаличиеАктивныхСоединений Тогда
				...
				ПараметрДействия = ОбщегоНазначения.ПараметрыДействияВажногоСообщения();
				ПараметрДействия.ИмяКоманды = "ОткрытьАктивныеПользователи";
				ПараметрДействия.Заголовок = НСтр("ru = 'Посмотреть список активных пользователей'");
				ПараметрДействия.Обработчик = "Подключаемый_АктивныеПользователи";
				
				Если СоединенияИнфо.НаличиеCOMСоединений Тогда
					ПараметрДействия.Заголовок = НСтр("ru = 'Посмотреть список активных подключений'");
					Сообщение = НСтр("ru = 'С программой сейчас работают внешние приложения. Для продолжения необходимо завершить их работу.'");
				ИначеЕсли СоединенияИнфо.НаличиеСоединенияКонфигуратором Тогда
					Сообщение = НСтр("ru = 'Открыт Конфигуратор. Для продолжения необходимо завершить работу Конфигуратора.'");
				Иначе
					Сообщение = НСтр("ru = 'С программой сейчас работают другие пользователи. Для продолжения необходимо завершить их работу.'");
				КонецЕсли;
				
				ВставитьВажноеСообщение(
					Элементы.АктивныеСоединенияФайловыйРежим,
					"",
					Сообщение,
					"Предупреждение",
					Элементы.АктивныеСоединенияФайловыйРежим.Имя,
					,
					ПараметрДействия);
		...
		Иначе
			...
			Если Инициализировать Тогда
				...
				Если ПустаяСтрока(ВыбранноеОбновление.Конфигурация.URLПорядокОбновления) Тогда
					ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(
						ЭтотОбъект,
						Элементы.ПорядокОбновленияСервер.Имя);
				Иначе
					Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
						НСтр("ru = 'Для продолжения необходимо ознакомиться с <a href=""%1"">порядком обновления</a>.'"),
						ВыбранноеОбновление.Конфигурация.URLПорядокОбновления);
					ВставитьВажноеСообщение(
						Элементы.ПорядокОбновленияСервер,
						"",
						Сообщение,
						"Предупреждение",
						Элементы.ПорядокОбновленияСервер.Имя);
			...
			ЕстьОтложенныеОбработчики =
				(ОбновлениеИнформационнойБазы.СтатусОтложенногоОбновления() = "СтатусНеВыполнено");
			
			Сообщение = Новый Массив();	// Массив из ФорматированнаяСтрока, Строка
			Сообщение.Добавить(
				НСтр("ru = 'Перед установкой обновления рекомендуется выполнить резервное копирование информационной базы.'"));
			Если ЕстьОтложенныеОбработчики Тогда
				Сообщение.Добавить(Символы.ПС);
				Сообщение.Добавить(Символы.ПС);
				Сообщение.Добавить(
					СтроковыеФункции.ФорматированнаяСтрока(
						НСтр("ru = 'В программе имеются <a href=""ОтложенныеОбработчики"">дополнительные процедуры обработки данных</a>, которые не были завершены
							|при переходе с предыдущего релиза. Они будут автоматически выполнены в процессе обновления
							|на новую версию. Это может существенно увеличить время обновления.'")));
				ОбработчикСсылок = "Подключаемый_НадписьОтложенныеОбработчикиОбработкаНавигационнойСсылки";
			Иначе
				ОбработчикСсылок = "";
			КонецЕсли;
			
			ВставитьВажноеСообщение(
				Элементы.ГруппаРезервнаяКопияСервер,
				"",
				Новый ФорматированнаяСтрока(Сообщение),
				"Информация",
				Элементы.ГруппаРезервнаяКопияСервер.Имя,
				ОбработчикСсылок);
			...
			Если РежимОбновления = 0 Тогда
				...
				Если СоединенияИнфо.НаличиеАктивныхСоединений Тогда
					...
					Если СоединенияИнфо.НаличиеCOMСоединений Тогда
						ПараметрДействия.Заголовок = НСтр("ru = 'Посмотреть список активных подключений'");
						Сообщение = НСтр("ru = 'С информационной базой сейчас работают другие пользователи, а также внешние приложения.
							|Их работа будет завершена автоматически.'");
					Иначе
						Сообщение = НСтр("ru = 'С программой сейчас работают другие пользователи.
							|Их работа будет завершена автоматически.'");
					КонецЕсли;
					
					ВставитьВажноеСообщение(
						Элементы.АктивныеСоединенияСерверныйРежим,
						"",
						Сообщение,
						"Предупреждение",
						Элементы.АктивныеСоединенияСерверныйРежим.Имя,
						,
						ПараметрДействия);
...
&НаКлиенте
Процедура ПараметрыУстановкиПлатформыЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	
	Если РезультатЗакрытия = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РежимУстановкиПлатформы                = РезультатЗакрытия.РежимУстановкиПлатформы;
	СохранятьДистрибутивыПлатформыВКаталог = РезультатЗакрытия.СохранятьДистрибутивыПлатформыВКаталог;
	КаталогСохраненияПлатформы             = РезультатЗакрытия.КаталогСохраненияПлатформы;
	
КонецПроцедуры

&НаСервере
Процедура ВставитьВажноеСообщение(
	Родитель,
	Заголовок,
	Сообщение,
	ТипСообщения,
	...
	ПараметрыВажногоСообщения.ТипСообщения = ТипСообщения;
	ПараметрыВажногоСообщения.Заголовок = Заголовок;
	ПараметрыВажногоСообщения.Сообщение = Сообщение;
	ПараметрыВажногоСообщения.Идентификатор = Идентификатор;
	ПараметрыВажногоСообщения.Скрываемый = Ложь;
	ПараметрыВажногоСообщения.Обработчик = Обработчик;
	ПараметрыВажногоСообщения.ПараметрыДействия = ПараметрыДействия;
	
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Родитель);
