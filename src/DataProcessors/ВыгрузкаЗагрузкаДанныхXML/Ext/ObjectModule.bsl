...
Процедура Инициализация(ИсключитьМетаданные = Неопределено) Экспорт
	...
	ДеревоМетаданных.Колонки.Добавить("Отбор", Новый ОписаниеТипов("ОтборКомпоновкиДанных"));
	ДеревоМетаданных.Колонки.Добавить("ОтборДоступен", Новый ОписаниеТипов("Булево"));
...
Функция ПодготовитьПостроительДляВыгрузки(СтрокаДереваМетаданных, СтрокаПолейДляВыборки = "")
	...
	ЕстьДопОтборы = СтрокаДереваМетаданных.Отбор.Элементы.Количество() > 0;
	...
	Если ЕстьДопОтборы Тогда
		СхемаКомпоновкиДанных = СхемаКомпоновкиДанных(СтрокаДереваМетаданных, ИтоговыйТекстЗапроса);
		...
		СкопироватьОтбор(КомпоновщикНастроек.Настройки.Отбор, СтрокаДереваМетаданных.Отбор);
		
		УстановитьЗначениеПараметраКД(КомпоновщикНастроек, "ДатаНачала", ДатаНачала);
		УстановитьЗначениеПараметраКД(КомпоновщикНастроек, "ДатаОкончания", ДатаОкончания);
		
		ГруппировкаДетальныеДанные = КомпоновщикНастроек.Настройки.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
		ГруппировкаДетальныеДанные.Использование = Истина;
		
		ПолеГруппировки = ГруппировкаДетальныеДанные.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
		...
		Для Каждого ДоступноеПолеВыбора Из КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.Элементы Цикл
			...
			ПолеВыбора = ГруппировкаДетальныеДанные.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		...
		ЗапросМакета = МакетКомпоновки.НаборыДанных.ОсновнойНабор.Запрос;
		Запрос = Новый Запрос(ЗапросМакета);
	...
	Если ЕстьДопОтборы Тогда
		Для Каждого ЗначениеПараметра Из МакетКомпоновки.ЗначенияПараметров Цикл
			ПостроительОтчета.Параметры.Вставить(ЗначениеПараметра.Имя, ЗначениеПараметра.Значение);
		КонецЦикла;
	Иначе
...
Процедура ПостроениеПоддереваОбъекта(ОбъектМД, ЭлементДЗ, ЭлементОписания)
	...
	Если ЭлементОписания.Менеджер <> Неопределено Тогда
		...
		ЭлементДЗ.ОтборДоступен = Истина;
...
Функция СхемаКомпоновкиДанных(СтрокаДереваМетаданных, ТекстЗапроса = "") Экспорт
	...
	НаборДанныхЗапрос.Имя = "ОсновнойНабор";
...
Процедура СкопироватьОтбор(Приемник, Источник) Экспорт
...
Процедура УстановитьЗначениеПараметраКД(КомпоновщикНастроек, ИмяПараметра, ЗначениеПараметра)
	
	ПараметрыДанных = КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы;
		
	ТекущийПараметрКД = ПараметрыДанных.Найти(Новый ПараметрКомпоновкиДанных(ИмяПараметра));
	
	Если ТекущийПараметрКД <> Неопределено Тогда
		ТекущийПараметрКД.Использование = Истина;
		ТекущийПараметрКД.Значение = ЗначениеПараметра;
	КонецЕсли;
