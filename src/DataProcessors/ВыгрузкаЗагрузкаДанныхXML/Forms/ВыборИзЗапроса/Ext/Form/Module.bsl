...
&НаКлиенте
Процедура ОткрытьКонструкторЗапросов(Команда)
	
#Если МобильныйКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Работа консоли запросов в мобильном клиенте не поддерживается.'"));
		Возврат;
#Иначе
		ТекстЗапросаВФорме = СокрЛП(ДокументТекстЗапроса.ПолучитьТекст());
...
&НаСервере
Процедура ВыполнитьЗапросНаСервере(Знач ТекстЗапроса)
...
Процедура ПослеЗакрытияКонструктораЗапроса(ТекстЗапросаВФорме, СтруктураПараметров) Экспорт
	...
	ДокументТекстЗапроса.УстановитьТекст(ТекстЗапросаВФорме);
