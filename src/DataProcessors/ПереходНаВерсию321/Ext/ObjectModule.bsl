...
Процедура ПриАнализеФормы(Параметры, ОписаниеОбъекта, ОписаниеФормы)
	...
	ПодключаемыеКоманды_ПриАнализеФормы(Параметры, ОписаниеОбъекта, ОписаниеФормы);
...
Процедура ПодключаемыеКоманды_ПриАнализеФормы(Параметры, ОписаниеОбъекта, ОписаниеФормы)
	
	ПолноеИмяСхемы  = ОписаниеФормы.Каталог + "Ext\Form.xml";
	ПолноеИмяМодуля = ОписаниеФормы.Каталог + "Ext\Form\Module.bsl";
	Модуль = ОписаниеМодуля(ПолноеИмяМодуля);
	
	Если Не ФайлСуществует(ПолноеИмяСхемы) Или Модуль = Неопределено Или Модуль.ТекстМодуля = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РегулярноеВыражение = "Процедура\s+([^(]*).*?ПриАктивизацииСтроки\(";
	ВсеМестаНахожденияПроцедур = СтрНайтиВсеПоРегулярномуВыражению(Модуль.ТекстМодуля, РегулярноеВыражение, Истина);
	
	Если ВсеМестаНахожденияПроцедур.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого РезультатПоиска Из ВсеМестаНахожденияПроцедур Цикл
		...
		ПроцедураМодуля = НайтиПроцедуруМодуля(Модуль, ИмяПроцедуры);
		Если ПроцедураМодуля = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ПроцедураСтрокой = БлокВСтроку(ПроцедураМодуля);
		Если СтрНайти(НРег(ПроцедураСтрокой), НРег("ПодключаемыеКомандыКлиент.НачатьОбновлениеКоманд")) = 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого ИмяТаблицыФормы Из ИменаТаблиц Цикл
		Результат = ПодключаемыеКоманды_РасстановкаСобытийВФорме(ОписаниеОбъекта, Модуль, ДокументDOM, Разыменователь, ИмяТаблицыФормы);
	...
	Если ТекстМодуляИзменен И ЗаписатьМодуль(Модуль)
	   И Параметры.ИзмененныеФайлы.Найти(ПолноеИмяМодуля) = Неопределено Тогда
...
Функция ПодключаемыеКоманды_РасстановкаСобытийВФорме(ОписаниеОбъекта, Модуль, ДокументDOM, Разыменователь, ИмяТаблицыФормы)
	
	Результат = Новый Структура("ТекстМодуляИзменен,XMLФормыИзменен", Ложь, Ложь);
	Если Не ЗначениеЗаполнено(ИмяТаблицыФормы) Тогда
		Возврат Результат;
	КонецЕсли;

	ТаблицаФормы = НайтиЭлементФормы(ДокументDOM, Разыменователь, ИмяТаблицыФормы);
	СобытиеТаблицыПриНаведении = НайтиДочернийУзел(ДокументDOM, Разыменователь, ТаблицаФормы, "xmlns:Events/xmlns:Event[@name='OnHover']");
	Если СобытиеТаблицыПриНаведении = Неопределено Тогда
		Результат.XMLФормыИзменен = Истина;
		
		ИмяСобытия = ИмяТаблицыФормы + "ПриНаведении";
		Номер = 0;
		Пока СтрНайти(НРег(Модуль.ТекстМодуля), НРег("Процедура " + ИмяСобытия + "(")) > 0 Цикл
			Номер = Номер + 1;
			ИмяСобытия = ИмяТаблицыФормы + "ПриНаведении" + Строка(Номер);
		КонецЦикла;
		ПроцедураМодуля = Неопределено;
		...
		СобытиеТаблицыПриНаведении = СобытияТаблицы.ДобавитьДочерний(НовыйЭлементDOM);
		СобытиеТаблицыПриНаведении.УстановитьАтрибут("name", "OnHover");
		СобытиеТаблицыПриНаведении.ТекстовоеСодержимое = ИмяСобытия;
	Иначе
		ИмяСобытия = СобытиеТаблицыПриНаведении.ТекстовоеСодержимое;
		ПроцедураМодуля = НайтиПроцедуруМодуля(Модуль, ИмяСобытия);
	КонецЕсли;
	...
	Если ПроцедураМодуля = Неопределено Тогда
		...
		Область = НайтиОбластьМодуля(Модуль, "ОбработчикиСобытийЭлементовТаблицыФормы" + ИмяТаблицыФормы);
		Если Область <> Неопределено Тогда
			ПроцедураМодуля = ДобавитьЭлементВКонец(Область);
		Иначе
			ИмяСобытияПриАктивизацииСтроки = ИмяТаблицыФормы + "ПриАктивизацииСтроки";
			ПроцедураПриАктивизацииСтроки = НайтиПроцедуруМодуля(Модуль, ИмяСобытия);
			Если ПроцедураПриАктивизацииСтроки <> Неопределено И ПроцедураПриАктивизацииСтроки.Родитель <> Неопределено Тогда 
				ПроцедураМодуля = ДобавитьЭлементВКонец(ПроцедураПриАктивизацииСтроки.Родитель);
			Иначе
				ПроцедураМодуля = ДобавитьЭлементВКонец(Модуль.Структура);
			КонецЕсли;
		КонецЕсли;
		
		ПроцедураМодуля.ИнструкцияПрепроцессора = "&НаКлиенте";
		ПроцедураМодуля.Заголовок = "Процедура "+ ИмяСобытия + "(Элемент, Строка, Поле)";
	...
	Иначе
		ТекстМодуля = БлокВСтроку(ПроцедураМодуля);
		Если СтрНайти(НРег(ТекстМодуля), НРег("ПодключаемыеКомандыКлиент.НачатьОбновлениеКоманд(")) = 0 Тогда
			Результат.ТекстМодуляИзменен = Истина;
			ДобавитьСтрокуВКонец(ПроцедураМодуля, Символы.Таб + Символы.ПС + ТекстДляВставки);
		КонецЕсли;
	КонецЕсли;
