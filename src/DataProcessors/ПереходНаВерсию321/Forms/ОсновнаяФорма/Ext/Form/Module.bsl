...
Функция ЗапуститьФоновоеЗадание()
	...
	Если Метаданные.Обработки.Содержит(МетаданныеОбработки) Тогда
		Задание = ДлительныеОперации.ВыполнитьВФоне("Обработка.ПереходНаВерсию321.МодульОбъекта.Внедрение",
			ПараметрыПроцедуры, ПараметрыЗадания);
	Иначе
		Задание = ДлительныеОперации.ВыполнитьВФоне("ВнешняяОбработка.ПереходНаВерсию321.МодульОбъекта.Внедрение",
			ПараметрыПроцедуры, ПараметрыЗадания);
	КонецЕсли;
