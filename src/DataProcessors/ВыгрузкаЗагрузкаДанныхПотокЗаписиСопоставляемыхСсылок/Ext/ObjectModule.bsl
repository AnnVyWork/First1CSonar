...
Перем ТекущееИмяКолонкиСИсходнойСсылкой; // Строка
Перем ТекущиеСсылки;
...
Процедура ЗаполнитьКолонкиТаблицыИсходныхСсылок()
	...
	ТекущиеСсылки.Колонки.Добавить(
		ТекущееИмяКолонкиСИсходнойСсылкой,
		ОбщегоНазначенияБТССлужебныйПовтИсп.ОписаниеСсылочныхТипов());
	...
	Если ТипыОбщихДанных.Найти(ТекущийОбъектМетаданных) <> Неопределено Тогда
		...
		Для Каждого ПолеЕстественногоКлюча Из ПоляЕстественногоКлюча Цикл
			...
			Для Каждого ВозможныйТип Из ОписаниеТиповПоля.Типы() Цикл
				...
				Если ОбщегоНазначенияБТС.ЭтоПримитивныйТип(ВозможныйТип)
					Или ВозможныйТип = Тип("ХранилищеЗначения") Тогда
			...
			Если ТолькоСсылочныеТипы Тогда
				ОписаниеТиповПоля = ОбщегоНазначенияБТССлужебныйПовтИсп.ОписаниеСсылочныхТипов();
			КонецЕсли;
