...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КлиентскоеПриложение.ТекущийВариантИнтерфейса() <> ВариантИнтерфейсаКлиентскогоПриложения.Версия8_5 Тогда
		...
		Элементы.ПользователиЭтоПользовательИБ.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
		Элементы.ПользователиЭтоПользовательСистемыВзаимодействия.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
		Элементы.ПользователиЗаблокирован.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
		Элементы.СвязиПриложенийПредставлениеСопоставленияПользователей.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
		Элементы.СвязиПриложенийСопоставлениеОбсуждений.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
		Элементы.ТаблицаСопоставленияПользовательИБ.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
		Элементы.ТаблицаСопоставленияПользовательСервиса.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;

		Элементы.ПользователиЭтоПользовательИБ.Заголовок = НСтр("ru='Информационная база'");
		Элементы.ПользователиЭтоПользовательСистемыВзаимодействия.Заголовок = НСтр("ru='Система взаимодействия'");
		Элементы.ПользователиЗаблокирован.Заголовок = НСтр("ru='Заблокирован'");
		Элементы.СвязиПриложенийПредставлениеСопоставленияПользователей.Заголовок = НСтр("ru='Пользователи'");
		Элементы.СвязиПриложенийСопоставлениеОбсуждений.Заголовок = НСтр("ru='Обсуждения'");
		Элементы.ТаблицаСопоставленияПользовательИБ.Заголовок = НСтр("ru='Пользователь локальной базы'");
		Элементы.ТаблицаСопоставленияПользовательСервиса.Заголовок = НСтр("ru='Пользователь сервиса'");
...
&НаКлиенте
Процедура ДобавитьСвязьПриложений(Команда)
	ИмяФормыСвязи = СтрШаблон("%1.Форма.ФормаСвязиПриложений", ИмяОбъектаОбработки);
	Оповещение = Новый ОписаниеОповещения("ФормаСвязиПриложенийЗакрыта", ЭтотОбъект);
	ФормаСвязи = ОткрытьФорму(ИмяФормыСвязи, , , , , , Оповещение);
	ФормаСвязи.Открыть();
...
Процедура РазблокироватьПользователя(Команда)
	...
	ВыбранныеСтроки = Элементы.ТаблицаПользователей.ПолучитьСтрокиДляОбработки();
...
Процедура УстановитьДоступностьКоманд()
	...
	ВыбранныеСтроки = Элементы.ТаблицаПользователей.ПолучитьСтрокиДляОбработки();
	Для Каждого ИдентификаторВыбраннойСтроки Из ВыбранныеСтроки Цикл
		Если Не ЗначениеЗаполнено(ИдентификаторВыбраннойСтроки) Тогда
			Продолжить;
		КонецЕсли;
		Строка = ТаблицаПользователей.НайтиПоИдентификатору(ИдентификаторВыбраннойСтроки);
