...
Функция ПрочитатьОбъектДанныхИнформационнойБазы() Экспорт
	...
	ИмяЭлементаВыгрузки = "DumpElement";		
	Если ПотокЧтения.Имя <> ИмяЭлементаВыгрузки Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента %1.'"), ИмяЭлементаВыгрузки);
	КонецЕсли;
	...
	Если ПропускатьОшибки Тогда
		...
		Исключение
			...
			ИсходноеИсключение = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПрочитатьПроблемныйОбъектИнформационнойБазы()
	...
	Если ПотокЧтения.Имя = "Artefacts" Тогда
		...
		Пока ПотокЧтения.ТипУзла <> ТипУзлаXML.КонецЭлемента Цикл
			...
			Исключение
				ИсходноеИсключение = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
