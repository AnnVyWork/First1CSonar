...
Процедура ОбъектМетаданныхОткрытие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму(Объект.ОбъектМетаданных + ".ФормаСписка", ПараметрыОткрытия,
...
Процедура Подключаемый_ТабличныйДокументПриАктивизацииОбласти(Элемент)
	...
	Если ИсходнаяОбласть = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура Подключаемый_ТабличныйДокументНачалоВводаЗначения(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли ЭтоОписаниеТипов(ТипРеквизитаСтруктуры) Тогда
		...
		ОткрытьФорму("ВнешняяОбработка.НачальноеЗаполнениеДанных.Форма.ФормаРедактированияОписанияТипов",
			ПараметрыОткрытия, ЭтотОбъект,,,, ОписаниеОповещенияОЗакрытии, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
	Иначе
		...
		ОткрытьФорму(МетаданныеРеквизитаПолноеИмя + СтрокаИмениФормы, ПараметрыОткрытия, ЭтотОбъект,,,,
			ОписаниеОповещенияОЗакрытии, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
	КонецЕсли;
...
Процедура ВывестиТабличныеЧастиТекущегоЭлемента()
	...
	Если НЕ ОтображатьТабличныеЧасти Или ТабличныйДокумент.ТекущаяОбласть = Неопределено Тогда
		Возврат;
	КонецЕсли;
