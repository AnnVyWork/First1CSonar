...
Процедура РассчитатьКоличествоДней(Команда)
	...
	Попытка
		РазностьДат = ПолучитьРазностьДатПоКалендарю(Календарь, ДатаНачала, ДатаОкончания);
		Элементы.РазностьДат.ФорматРедактирования = ФорматПредставленияДня();
	Исключение
		ОбщегоНазначенияКлиент.СообщитьПользователю(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), Календарь);
	КонецПопытки;
...
//
&НаСервереБезКонтекста
Функция ПолучитьРазностьДатПоКалендарю(Календарь, ДатаНачала, ДатаОкончания)
...
&НаКлиенте
Функция ФорматПредставленияДня()
	
	ПредставлениеДня = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		НСтр("ru = ';день;;дня;дней;дня'"),
		РазностьДат);
	
	ФорматПредставленияДня = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"ЧФ='Ч %1'",
		ПредставлениеДня);
	
	Возврат ФорматПредставленияДня;
