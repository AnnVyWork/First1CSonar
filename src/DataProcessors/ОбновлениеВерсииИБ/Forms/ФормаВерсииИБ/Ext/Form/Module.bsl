...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Элементы.ВерсииОбщихДанных.Заголовок = НСтр("ru = 'Версии общих данных'");
		
		Если НЕ СеансЗапущенБезРазделителей Тогда
			Элементы.Версии.ТолькоПросмотр = Истина;
		КонецЕсли;
	Иначе
		Элементы.ВерсииОбластейДанных.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПерезапуститьПрограмму(Команда)
	...
	КнопкиВопроса.Добавить("ПерезапуститьСОбновлением", НСтр("ru = 'Перезапустить с параметром ЗапуститьОбновлениеИнформационнойБазы'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ru = 'Отмена'"));
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Перезапустить приложение?'"), КнопкиВопроса);
...
Процедура ПерезапуститьПрограммуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = "ПерезапуститьСОбновлением" Тогда
		ЗапуститьОбновлениеИнформационнойБазы = Истина;
	КонецЕсли;
