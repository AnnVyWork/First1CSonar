...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЭтоВариантИнтерфейса85() Тогда
		Элементы.Переместить(Элементы.ГруппаКнопокСохранения, Элементы.ГруппаКоманднаяПанель);
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	ОтобразитьВкладкиСогласноНастройкам(Настройки["Объект.ИспользоватьВкладки"]);
	
КонецПроцедуры

&НаСервере
Процедура ОтобразитьВкладкиСогласноНастройкам(ИспользоватьВкладки)
	
	Если ИспользоватьВкладки И Элементы.Страницы.ПодчиненныеЭлементы.Количество() = 0 Тогда
		Элементы.Переместить(Элементы.ГруппаПараметрыЗапроса, Элементы.Страницы);
		Элементы.Переместить(Элементы.ГруппаТекстЗапроса,     Элементы.Страницы);
		Элементы.Переместить(Элементы.ГруппаРезультатЗапроса, Элементы.Страницы);
	Иначе
		Элементы.Переместить(Элементы.ГруппаПараметрыЗапроса, Элементы.Документ);
		Элементы.Переместить(Элементы.ГруппаТекстЗапроса,     Элементы.Документ);
		Элементы.Переместить(Элементы.ГруппаРезультатЗапроса, Элементы.Документ);
		Элементы.ГруппаПараметрыЗапроса.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
		Элементы.ГруппаТекстЗапроса.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
		Элементы.ГруппаРезультатЗапроса.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
	КонецЕсли;
...
Процедура ВыгрузитьНастройки(ПараметрыПередачи)
	...
	Объект.ИспользоватьВкладки                       = ПолучитьИзВременногоХранилища(ПараметрыПередачи.АдресХранилища).ИспользоватьВкладки;
	...
	ОтобразитьВкладкиСогласноНастройкам(Объект.ИспользоватьВкладки);
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	Если Объект.ИспользоватьВкладки Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаРезультатЗапроса;
	Иначе
...
Процедура ЗаполнитьПараметрыКлиент()
	...
	Если Объект.ИспользоватьВкладки Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаПараметрыЗапроса;
	Иначе
...
//
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоВариантИнтерфейса85()
#Если МобильныйКлиент Тогда
	Возврат Истина;
#КонецЕсли
