...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ИнтернетПоддержкаПользователейКлиентСервер.ЭтоИнтерфейс85() Тогда
		...
		Элементы.СправкаТакси.Видимость = Истина;
	КонецЕсли;
	...
	Если (ИмяСобытия = "ИнтернетПоддержкаОтключена"
		...
		Если ИмяСобытия = "ИнтернетПоддержкаОтключена" Тогда
			Если Результат.ВариантОбновления = 1 Тогда
				Элементы.ГруппаКлассификаторыПредупреждения.Видимость = Истина;
			КонецЕсли;
		ИначеЕсли ИмяСобытия = "ИнтернетПоддержкаПодключена" Тогда
			...
			Элементы.ГруппаКлассификаторыПредупреждения.Видимость = Ложь;
		КонецЕсли;
...
Процедура ВариантОбновленияКлассификаторовПриИзменении(Элемент)
	...
	Элементы.ФайлКлассификаторов.Доступность                          = (ВариантОбновленияКлассификаторов = 2);
	Элементы.ДекорацияРасписаниеОбновленияКлассификаторов.Доступность = (ВариантОбновленияКлассификаторов > 0);
...
Процедура ВариантОбновленияРегламентированныхОтчетовПриИзменении(Элемент)
	...
	Если ВариантОбновленияРегламентированныхОтчетов = 1
		...
		Возврат;
	Иначе
		Элементы.ГруппаРегламентированныеОтчетыПредупреждения.Видимость = Ложь;
	КонецЕсли;
	
	Элементы.ФайлРегламентированныхОтчетов.Доступность                          =
		(ВариантОбновленияРегламентированныхОтчетов = 2);
	Элементы.ДекорацияРасписаниеОбновленияРегламентированныхОтчетов.Доступность =
		(ВариантОбновленияРегламентированныхОтчетов > 0);
...
Процедура ВариантОбновленияВнешнихКомпонентПриИзменении(Элемент)
	...
	Если ВариантОбновленияВнешнихКомпонент = 1
		...
		Возврат;
	Иначе
		Элементы.ГруппаВнешниеКомпонентыПредупреждения.Видимость = Ложь;
	КонецЕсли;
	
	Элементы.ФайлВнешнихКомпонент.Доступность                          = (ВариантОбновленияВнешнихКомпонент = 2);
	Элементы.ДекорацияРасписаниеОбновленияВнешнихКомпонент.Доступность = (ВариантОбновленияВнешнихКомпонент > 0);
...
&НаКлиенте
Процедура ОткрытьНастройкиПодключенияОнлайнЗаказов(Команда)
...
Процедура ОбновитьСостояниеИнтернетПоддержки()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.РаботаСКлассификаторами") Тогда
		...
		Если ПараметрыПодсистемыРаботыСКлассификаторами.ИнтерактивнаяЗагрузкаДоступна Тогда
			...
			Элементы.ГруппаКлассификаторыПредупреждения.Видимость =
...
Процедура ОтобразитьСостояниеПодключенияИПП(Форма, ЗаголовокПодключения)
	...
	Если ДанныеАутентификации = Неопределено Тогда
		...
		Элементы.ВойтиИлиВыйтиИПП.Важность             = ВажностьКнопки.Главная;
	Иначе
		Элементы.ДекорацияЛогинИПП.Заголовок           = ЗаголовокПодключения;
		...
		Элементы.ВойтиИлиВыйтиИПП.Важность             = ВажностьКнопки.Обычная;
	КонецЕсли;
...
Процедура НастроитьОтображениеБлокировкиРаботыССервисамиИнтернетПоддержки()
	...
	Сообщение = ?(Пользователи.ЭтоПолноправныйПользователь(),
		СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Работа с сервисами Интернет-поддержки заблокирована для предотвращения конфликтов с основной информационной базой. <a href=%1>Настроить</a>.'"),
			"SetupAccess"),
	...
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Работа с сервисами заблокирована.'");
	ПараметрыСообщения.Скрываемый = Истина;
	ПараметрыСообщения.Идентификатор = "РаботаСВнешнимиРесурсамиЗаблокирована";
	ПараметрыСообщения.Обработчик = "ПояснениеБлокировкаРаботыССервисамиИнтернетПоддержкиОбработкаНавигационнойСсылки";
	
	ОбщегоНазначения.ДобавитьВажноеСообщение(
		ЭтотОбъект,
		ПараметрыСообщения);
...
&НаСервере
Процедура НастроитьОтображениеБлокировкиИнтернетСервисов(
		Знач РазрешитьДоступКИнтернетСервисам = Неопределено)
	...
	Если Не РазделениеВключено И Не РазрешитьДоступКИнтернетСервисам Тогда
		Сообщение = ?(Пользователи.ЭтоПолноправныйПользователь(),
		СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'В программе заблокирована работа с Интернет-сервисами. <a href=%1>Разблокировать</a>.'"),
			"UnlockServices"),
		НСтр("ru = 'В программе заблокирована работа с Интернет-сервисами. Для разблокировки обратитесь к администратору.'"));
	
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Сообщение = Сообщение;
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Доступ к сервисам отключен.'");
		ПараметрыСообщения.Скрываемый = Истина;
		ПараметрыСообщения.Идентификатор = "РаботаССервисамиЗаблокирована";
		ПараметрыСообщения.Обработчик = "НадписьБлокировкаРаботыИнтернетСервисовОбработкаНавигационнойСсылки";
		
		ОбщегоНазначения.ДобавитьВажноеСообщение(
			ЭтотОбъект,
			ПараметрыСообщения);
		ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму(ЭтотОбъект);
	ИначеЕсли Не РазделениеВключено И РазрешитьДоступКИнтернетСервисам Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(
			ЭтотОбъект,
			"РаботаССервисамиЗаблокирована");
	КонецЕсли;
