...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.РассчитатьРазмерПриложения.Вид = ВидКнопкиФормы.Гиперссылка;
	КонецЕсли;
	
	РазмерПриложенийСлужебный.ПроверитьПоддержкуРасчетаРазмераПриложений();
...
Процедура ПодключитьОбработчикОжиданияВыполненияРасчета(УвеличитьИнтервал = 0)
	...
	ИнтервалОжиданияВыполненияРасчета = Мин(5 + УвеличитьИнтервал, 300);
...
Процедура ЗапланироватьРасчетРазмераПриложения()
	...
	РазмерПриложенийСлужебный.ЗапланироватьРасчетРазмераПриложения();
...
Процедура ОбновитьВидимостьГруппыРазмераПриложения()
	...
	Элементы.ИнфоАктуальностьРасчета.Видимость = ЭтоРазделенныйСеанс();
	МинимальныйШагИзменений = РазмерПриложенийСлужебный.ЗначениеНастройкиРасчета("МинимальныйШагИзменений", 0);
...
Процедура ОбновитьАктуальностьРасчетаПриложения()
	...
	ДатаРасчета = РазмерПриложенийСлужебный.АктуальностьРасчетаРазмераПриложения();
...
Функция ЕстьЗапланированноеЗаданиеРасчетаРазмераПриложения()
	...
	Возврат РазмерПриложенийСлужебный.ЕстьЗапланированноеЗаданиеРасчетаРазмераПриложения();
