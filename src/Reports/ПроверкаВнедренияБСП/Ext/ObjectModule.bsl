	...
	Для Каждого ВидВоМножественномЧисле Из МассивВидов Цикл
		...
		Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
			...
			Для Каждого Форма Из ОбъектМетаданных.Формы Цикл // ОбъектМетаданныхФорма
				...
				ОписаниеВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'вызов %1'"), "ПодключаемыеКоманды.ПриСозданииНаСервере");
				...
				Если ЭтоФормаОбъекта Тогда
					ОписаниеВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'вызов %1'"), "ПодключаемыеКомандыКлиент.ПослеЗаписи");
				...
				РегулярноеВыражение = "Процедура\s+([^(]*).*?ПриАктивизацииСтроки\(";
				Если Не ЗначениеЗаполнено(ТекстМодуляФормы) Тогда
					ВсеМестаНахожденияПроцедур = Новый Массив;
				Иначе
					ВсеМестаНахожденияПроцедур = СтрНайтиВсеПоРегулярномуВыражению(ТекстМодуляФормы, РегулярноеВыражение, Истина);
				КонецЕсли;
				ИменаТаблиц = Новый Массив;
				Для Каждого РезультатПоиска Из ВсеМестаНахожденияПроцедур Цикл
					ИмяПроцедуры = РезультатПоиска.Значение;
					ИмяПроцедуры = СтрЗаменить(ИмяПроцедуры, "Процедура", "");
					ИмяПроцедуры = СтрЗаменить(ИмяПроцедуры, "(", "");
					ИмяПроцедуры = СокрЛП(ИмяПроцедуры);
					ПроцедураМодуля = НайтиМетод(ТекстМодуляФормы, ИмяПроцедуры);
					Если ПроцедураМодуля = Неопределено Тогда
						Продолжить;
					КонецЕсли;
					Если НайтиВызовМетода(ПроцедураМодуля.Содержимое, "ПодключаемыеКомандыКлиент.НачатьОбновлениеКоманд(") = Неопределено Тогда
						Продолжить;
					КонецЕсли;
					ИмяТаблицы = ИмяПроцедуры;
					Если Не ЗначениеЗаполнено(ИмяТаблицы) Тогда
						Продолжить;
					КонецЕсли;
					ИмяТаблицы = СтрЗаменить(ИмяТаблицы, "ПриАктивизацииСтроки", "");
					ИменаТаблиц.Добавить(ИмяТаблицы);
				КонецЦикла;
				Для Каждого ИмяТаблицыФормы Из ИменаТаблиц Цикл
					ИмяСобытия = ИмяТаблицыФормы + "ПриНаведении";
					ПриНаведении = НайтиМетод(ТекстМодуляФормы, ИмяСобытия);
					Если ПриНаведении <> Неопределено Тогда
						ОписаниеВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'вызов %1 в %2'"),
							"ПодключаемыеКомандыКлиент.НачатьОбновлениеКоманд", ИмяСобытия);
						Если НайтиВызовМетода(ПриНаведении.Содержимое, "ПодключаемыеКомандыКлиент.НачатьОбновлениеКоманд(") = Неопределено Тогда
							Отсутствующие.Добавить(ОписаниеВставки);
						Иначе
							Присутствующие.Добавить(ОписаниеВставки);
						КонецЕсли;
					Иначе
						ОписаниеВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'обработка события ""ПриНаведении"" для элемента ""%1"" с вызовом %2'"),
							ИмяТаблицыФормы, "ПодключаемыеКомандыКлиент.НачатьОбновлениеКоманд");
						Отсутствующие.Добавить(ОписаниеВставки);
					КонецЕсли;
				КонецЦикла;
...
Процедура ПроверитьНаличиеВставкиКодаДляОбъекта(ПараметрыПроверки)
	...
	Если ТипЗнч(СтрокаКода) = Тип("Массив") Тогда
		Для Каждого СтрокаВызова Из СтрокаКода Цикл
			...
			ПроверитьНаличиеЭлементаФормыДляРазмещенияРеквизитов(ТекстМодуля, СтрокаВызова, ОбъектМетаданных);
		КонецЦикла;
		Возврат;
	КонецЕсли;
...
Процедура ПереключитьРазделение(Значение)
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Менеджер = Константы.ИспользоватьРазделениеПоОбластямДанных.СоздатьМенеджерЗначения();
...
Процедура ПроверитьНаличиеЭлементаФормыДляРазмещенияРеквизитов(ТекстМодуля, СтрокаВызова, ОбъектМетаданных)
	
	Если СтрСравнить(ПроверяемаяПодсистема, "Свойства") <> 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если СтрНайти(ВРег(СтрокаВызова), ВРег(".ПриСозданииНаСервере(")) = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТекстПроцедурыПриСозданииНаСервере = ТекстПроцедуры("ПриСозданииНаСервере", ТекстМодуля);
	Если Не ЗначениеЗаполнено(ТекстПроцедурыПриСозданииНаСервере) Тогда
		Возврат;
	КонецЕсли;
	
	МассивЭлементовДляПоиска = Новый Массив;
	ВыражениеПоиска = "\.Вставить\(\s*""ИмяЭлементаДляРазмещения""\s*,\s*(""?\w+""?)\s*\)";
	ШаблонВыраженияПоискаИнициализации = "\b%1\s*=([^;]*)";
	ШаблонВыраженияПоискаДобавленияВСписокЗначений = "\b%1\.Добавить\([^,]*,\s+([\w\.""]*)\);";
	
	РезультатПоиска = СтрНайтиВсеПоРегулярномуВыражению(ТекстПроцедурыПриСозданииНаСервере, ВыражениеПоиска, Истина, Истина);
	Для Каждого НайденноеВыражение Из РезультатПоиска Цикл
		
		ГруппыНайденногоВыражения = НайденноеВыражение.ПолучитьГруппы();
		Если ГруппыНайденногоВыражения.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		НаименованиеГруппы = СокрЛП(ГруппыНайденногоВыражения[0].Значение);
		
		Если СтрНачинаетсяС(НаименованиеГруппы, """") Тогда
			НаименованиеГруппы = СтрЗаменить(НаименованиеГруппы, """", "");
			МассивЭлементовДляПоиска.Добавить(НаименованиеГруппы);
		Иначе
			// Это переменная. Определяем место инициализации.
			ВыражениеПоискаИнициализации = СтрШаблон(ШаблонВыраженияПоискаИнициализации, НаименованиеГруппы);
			РезультатПоискаИнициализации = СтрНайтиПоРегулярномуВыражению(ТекстПроцедурыПриСозданииНаСервере,
				ВыражениеПоискаИнициализации,,,, Истина, Истина);
			
			ЗначениеПеременной = "";
			Если РезультатПоискаИнициализации.НачальнаяПозиция < НайденноеВыражение.НачальнаяПозиция Тогда
				НайденныеГруппы = РезультатПоискаИнициализации.ПолучитьГруппы();
				Если НайденныеГруппы.Количество() > 0 Тогда
					ЗначениеПеременной = НайденныеГруппы[0].Значение;
				КонецЕсли;
			КонецЕсли;
			
			ЗначениеПеременной = СокрЛП(ЗначениеПеременной);
			
			Если СтрСравнить(ЗначениеПеременной, "Новый СписокЗначений") = 0 Тогда
				
				ВыражениеПоискаДобавленияВСписок = СтрШаблон(ШаблонВыраженияПоискаДобавленияВСписокЗначений, НаименованиеГруппы);
				РезультатПоискаДобавленияВСписок = СтрНайтиВсеПоРегулярномуВыражению(ТекстПроцедурыПриСозданииНаСервере,
					ВыражениеПоискаДобавленияВСписок, Истина, Истина);
				
				Для Каждого РезультатПоиска Из РезультатПоискаДобавленияВСписок Цикл
					...
					НайденноеЗначение = НайденныеГруппы[0].Значение;
					ИмяГруппы = "";
					
					Если СтрНачинаетсяС(НайденноеЗначение, "Элементы.") Тогда
						МассивПодстрок = СтрРазделить(НайденноеЗначение, ".");
						ИмяГруппы = МассивПодстрок[1];
					КонецЕсли;
					
					Если СтрНачинаетсяС(НайденноеЗначение, """") Тогда
						ИмяГруппы = СтрЗаменить(НайденноеЗначение, """", "");
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ИмяГруппы) Тогда
						МассивЭлементовДляПоиска.Добавить(ИмяГруппы);
					КонецЕсли;
					
				КонецЦикла;
				
			ИначеЕсли НЕ ПустаяСтрока(ЗначениеПеременной) Тогда
				
				МассивЭлементовДляПоиска.Добавить(ЗначениеПеременной);
	...
	Если МассивЭлементовДляПоиска.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПутьКФайлуОписанияЭлементовФормы = ПутьКФайлуОписанияЭлементовФормы(ОбъектМетаданных);
	Если НЕ ФайлСуществует(ПутьКФайлуОписанияЭлементовФормы) Тогда
		Возврат;
	КонецЕсли;
	
	ТекстОшибкиКратко = НСтр("ru='Не найдена группа формы'");
	ШаблонТекстОшибкиПодробно = НСтр("ru='На форме ""%1"" не найдена группа ""%2"" для размещения дополнительных реквизитов'");
	
	ДокументDOM = ДокументDOM(ПутьКФайлуОписанияЭлементовФормы);
	Если КонфигурацияEDT Тогда
		ШаблонВыраженияXPath = "//name[text() = '%1']";
	Иначе
		ШаблонВыраженияXPath = "//*[@name = '%1']";
	КонецЕсли;
	
	Для Каждого ЭлементДляПоиска Из МассивЭлементовДляПоиска Цикл
		
		ВыражениеXPath = СтрШаблон(ШаблонВыраженияXPath, ЭлементДляПоиска);
		...
		ГруппаФормыНайдена = (ЭлементDOMЭлементаФормы <> Неопределено);
		
		Если Не ГруппаФормыНайдена Тогда
			ТекстОшибкиПодробно = СтрШаблон(ШаблонТекстОшибкиПодробно, ОбъектМетаданных.Имя, ЭлементДляПоиска);
			ДобавитьОшибку(ОбъектМетаданных, ТекстОшибкиКратко, ТекстОшибкиПодробно);
		КонецЕсли;
