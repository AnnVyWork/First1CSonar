...
Процедура ВыполнитьПодготовительныеДействия()
	...
	ЗависимостиПодсистем.Индексы.Добавить("Имя");
	ЗаполнитьИсключения();
...
Процедура ЗаполнитьПринадлежностьОбъектаПодсистеме(ПринадлежностьОбъектаПодсистеме, ПолноеИмяОМД, Подсистема)
	...
	ИначеЕсли СтрНайти(ПолноеИмяОМД, "ЦветПалитры.") > 0 Тогда
		
		Возврат;
		
	ИначеЕсли СтрНайти(ПолноеИмяОМД, "СервисИнтеграции.")
		> 0 Тогда
...
Процедура ВыполнитьПоискВызововВДругиеПодсистемы(СтруктураПоискаВызова)
	...
	Иначе
		Если СтрНайти(СтрокаТекста, СтрокаОбращенияИзКода) = 0 Тогда
			Возврат;
		КонецЕсли;
...
Процедура НайтиОбращениеКМетаданнымВКодеМодулей(СтруктураПоискаВызова)
	...
	Если СтрНайти(СтруктураПоискаВызова.СтрокаТекста, СтруктураПоискаВызова.ЭлементСоответствия.ИмяОМД) = 0 Тогда
		Возврат;
	КонецЕсли;
