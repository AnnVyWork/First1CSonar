...
Процедура ОбратныйОтсчетКодТелефона()
	...
	ИначеЕсли ОтключитьОбратныйОтсчет <> 99 Тогда
		...
		Иначе
			...
			УстановитьЦветСтатусаКодаПоПалитре("Красный");
		КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.СтатусКода = НСтр("ru = 'Указан верный пароль'") Тогда
		Элементы.КартинкаТелефонПроверен.Видимость = Истина;
	КонецЕсли;   
	
	Если ЗначениеЗаполнено(Форма.Получатель) Тогда
		Элементы.ДекорацияТелефон.Заголовок = НСтр("ru = 'Пароль отправлен на номер:'") + " " + Строка(Форма.Получатель);
	КонецЕсли;    
	
	Элементы.ПарольПользователя.Заголовок = НСтр("ru = 'Пароль пользователя:'") + " " + Строка(Форма.Логин);
...
&НаСервере
Процедура УстановитьЦветСтатусаКодаПоПалитре(Цвет)  
		
	Элементы.СтатусКода.ЦветТекста = ЦветаПалитры[Цвет];
