...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЭтоВебКлиент Тогда
		Элементы.ГруппаРаботаВВебКлиенте.Видимость = Ложь;
	КонецЕсли;
	...
	ИспользоватьТекущуюДатуКомпьютера = Не ЗначениеЗаполнено(ЗначениеРабочейДаты);
	Элементы.ЗначениеРабочейДаты.Доступность = Не ИспользоватьТекущуюДатуКомпьютера;
	// Конец СтандартныеПодсистемы.БазоваяФункциональность
	
	// СтандартныеПодсистемы.Пользователи
	АвторизованныйПользователь = Пользователи.АвторизованныйПользователь();
	Если Не ПравоДоступа("Просмотр", Метаданные.НайтиПоТипу(ТипЗнч(АвторизованныйПользователь))) Тогда
		Элементы.АвторизованныйПользователь.Видимость = Ложь;
	КонецЕсли;
	...
	Элементы.ВариантИнтерфейса.СписокВыбора.Добавить("Такси", НСтр("ru = 'Интерфейс ""Такси""'"));
	Элементы.ВариантИнтерфейса.СписокВыбора.Добавить("Версия8_5", НСтр("ru = 'Интерфейс версии 8.5'"));
	
	НастройкиКлиентскогоПриложения = ОбщегоНазначения.ХранилищеСистемныхНастроекЗагрузить("Общее/НастройкиКлиентскогоПриложения", "");
	Если НастройкиКлиентскогоПриложения.ВариантИнтерфейсаКлиентскогоПриложения = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_5 Тогда
		ВариантИнтерфейса = "Версия8_5";
	Иначе
		ВариантИнтерфейса = "Такси";
	КонецЕсли;
...
Процедура ОбновитьГруппуРаботыВВебКлиентеЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Элементы.СтраницыРаботаВВебКлиенте.ТекущаяСтраница =
		?(Подключено, Элементы.ГруппаРасширениеУстановлено, Элементы.ГруппаРасширениеНеУстановлено);
...
Процедура ИспользоватьТекущуюДатуКомпьютераПриИзменении(Элемент)
	...
	Если ИспользоватьТекущуюДатуКомпьютера Тогда
...
Функция УстановитьНастройкиНаСервере(Настройки)
	...
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить("Общее/НастройкиКлиентскогоПриложения", "", НастройкиКлиентскогоПриложения);
