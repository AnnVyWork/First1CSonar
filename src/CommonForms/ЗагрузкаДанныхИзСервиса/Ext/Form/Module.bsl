...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ПрерванаПроцедураЗагрузки Тогда
		ЗаполнитьПараметрыПрерваннойЗагрузки();
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ПродолжитьПрерваннуюЗагрузкуПриНеобходимости();
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы 
		Или Не (УстановленаМонопольнаяБлокировка 
			Или ЗначениеЗаполнено(ИдентификаторЗадания) 
			Или ЗагрузкаПрерванаВРезультатеОшибки) Тогда
		Возврат;
	КонецЕсли;
	
	Отказ = Истина;
	
	Если ЗначениеЗаполнено(ИдентификаторЗадания) Или ЗагрузкаПрерванаВРезультатеОшибки Тогда
		
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ВопросПередЗакрытиемЗавершение", ЭтотОбъект),
			НСтр("ru = 'Загрузка данных не завершена.
				|Вы можете прервать загрузку и продолжить позже.
				|Прервать загрузку данных?'"),
...
&НаКлиенте
Процедура ОшибкаФайлаДанныхОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Элементы.ГруппаИнформацияОбОшибкахФайлаДанных.Показать();
	ТекущийЭлемент = Элементы.ИнформацияОбОшибкахФайлаДанных;
...
&НаКлиенте
Процедура СписокПользователейПриНаведении(Элемент, Строка, Поле)
	...
	Если Строка = Неопределено Тогда
		Элементы.СписокПользователейЗавершитьСеансы.Доступность = Истина;
		Возврат;
	КонецЕсли;
	...
	ТекущийСеанс = СписокПользователей.НайтиПоИдентификатору(Строка);
	
	Если ТекущийСеанс = Неопределено Тогда
		Элементы.СписокПользователейЗавершитьСеансы.Доступность = Истина;
	Иначе
		Элементы.СписокПользователейЗавершитьСеансы.Доступность = 
			ТекущийСеанс.Сеанс <> НомерСеансаИнформационнойБазы;
	КонецЕсли;
...
Процедура ПродолжитьЗагрузку(Команда)
	...
	Если Элементы.СтраницыМастераЗагрузки.ТекущаяСтраница = Элементы.СтраницаОшибкиПредупреждения Тогда
		СброситьДанныеФормы();
		УстановитьМонопольнуюБлокировкуИПродолжитьЗагрузку();
		ЗаполнитьПараметрыПрерваннойЗагрузки();
		ПродолжитьПрерваннуюЗагрузкуПриНеобходимости();
...
Процедура ОткрытьДополнительныеПараметры(Команда)
	Если Не ДополнительныеПараметрыОткрыта Тогда
		Элементы.ГруппаДополнительныеПараметры.Показать();
		ТекущийЭлемент = Элементы.ЗагружатьРасширенияКонфигурации;
	Иначе
		Элементы.ГруппаДополнительныеПараметры.Скрыть();
	КонецЕсли;
	ДополнительныеПараметрыОткрыта = Не ДополнительныеПараметрыОткрыта;
КонецПроцедуры   
...
Процедура ВыбратьФайлДанныхВВебКлиенте(ЗаголовокДиалога = Неопределено)
	...
	ПараметрыПередачи = ФайлыБТССлужебныйКлиент.ПараметрыПомещенияФайла();
	...
	ФайлыБТССлужебныйКлиент.ПоместитьФайлИнтерактивно(ПараметрыПередачи);
...
Асинх Процедура ПослеПроверкиФайлаДанных(РезультатПроверки)
	...
	Элементы.ГруппаИнформацияОбОшибкахФайлаДанных.Скрыть();
	Элементы.ГруппаОшибкиФайлаДанных.Видимость = Ложь;
	СброситьЦвет(Элементы.ФайлДанных.ЦветРамки);
	ИнформацияОбОшибкахФайлаДанных = "";
	
	Если ЗначениеЗаполнено(РезультатПроверки.ТекстОшибки) Тогда
		
		ЧастиОшибки = Новый Массив;
		ЧастиОшибки.Добавить(Новый ФорматированнаяСтрока(РезультатПроверки.ТекстОшибки));
		Если ЗначениеЗаполнено(РезультатПроверки.ИнформацияОбОшибке) Тогда
			ЧастиОшибки.Добавить(Символы.ПС);
			ЧастиОшибки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Информация об ошибках'"),,,,"ПоказатьОшибки"));
			ИнформацияОбОшибкахФайлаДанных = РезультатПроверки.ИнформацияОбОшибке;
		КонецЕсли;
		Элементы.ДекорацияОшибкаФайлаДанных.Заголовок = Новый ФорматированнаяСтрока(ЧастиОшибки);
		Элементы.ГруппаОшибкиФайлаДанных.Видимость = Истина;
		УстановитьКрасныйЦвет(Элементы.ФайлДанных.ЦветРамки);
...
Асинх Функция ПоместитьФайлДанныхНаСервер()
	...
	Попытка
		...
		ФайлДанныхПомещен = Ждать ФайлыБТССлужебныйКлиент.ПоместитьФайлНаСервер(
...
Функция ПроверитьВозможностьЗагрузкиФайлаДанныхВВебКлиенте(Знач ИмяВременногоФайла)
	...
	ПолноеИмяВременногоФайла = ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(ИмяВременногоФайла);
...
Функция ПроверитьВозможностьЗагрузкиФайлаДанных(Знач ДанныеПроверяемыхФайлов)
	...
	Если Не ВыгрузкаЗагрузкаДанныхСлужебный.ВыгрузкаДанныхСовместимаСТекущейКонфигурацией(ИнформацияОВыгрузке) Тогда
	...
	ИначеЕсли Не ВыгрузкаЗагрузкаДанныхСлужебный.ВыгрузкаДанныхСовместимаСТекущейВерсиейКонфигурации(ИнформацияОВыгрузке) Тогда
...
Процедура СравнитьСхемыКонфигураций(ПутьКФайлуСхемыКонфигурацииВыгрузки, РезультатПроверки)
	...
	ОписанияРазличий = АнализСхемКонфигурацииСлужебный.ОписанияРазличийСхемКонфигураций(
...
Функция ПолучитьДанныеФайловДляПроверкиЗагрузкиДанных(ПолноеИмяФайлаДанных)
	...
	ЧтениеАрхива = Новый ЧтениеФайлаАрхива(ПолноеИмяФайлаДанных);
	
	Попытка
		
		ИзвлечьПроверяемыеФайлыИзАрхива(ЧтениеАрхива, ДанныеПроверяемыхФайлов);
...
// 
// Параметры:
//  ЧтениеАрхива - ЧтениеФайлаАрхива
//  ДанныеПроверяемыхФайлов - см. НовыеДанныеПроверяемыхФайлов
&НаКлиентеНаСервереБезКонтекста
Процедура ИзвлечьПроверяемыеФайлыИзАрхива(ЧтениеАрхива, ДанныеПроверяемыхФайлов)
	
	ИменаСлужебныхФайлов = ПолучитьИменаСлужебныхФайлов();
	ПутьКФайлуСодержимого = ИзвлечьФайлИзZipАрхива(ЧтениеАрхива, ИменаСлужебныхФайлов.Содержимое);
...
&НаКлиентеНаСервереБезКонтекста
Функция ИзвлечьФайлИзZipАрхива(ЧтениеАрхива, ИмяФайла)
	
	ИмяВременногоФайла = Неопределено;
	
	#Если НЕ ВебКлиент Тогда
		
	ЭлементАрхива = ЧтениеАрхива.Элементы.Найти(ИмяФайла);
...
Функция ПолучитьФайлВременногоХранилища(Знач ИдентификаторФормы, Знач ПолучатьПолноеИмя = Ложь)
	...
	ИмяФайла = ФайлыБТССлужебный.НовыйФайлВременногоХранилища("xml2data", "zip", 120);
	ФайлыБТССлужебный.ЗаблокироватьФайлВременногоХранилища(ИмяФайла, ИдентификаторФормы);
	...
	Если ПолучатьПолноеИмя Тогда
		Результат.ПолноеИмяФайла = ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(ИмяФайла);
	КонецЕсли;
...
Процедура ПроверитьГотовностьЗагрузки()
	...
	Исключение
		...
		ЗагрузкаПрерванаВРезультатеОшибки = Истина;
	...
	Элементы.СостояниеКартинка.Картинка = БиблиотекаКартинок.ДлительнаяОперация48; 
...
Функция СостояниеЗагрузки(ИдентификаторЗадания, ИдентификаторСостояния, АдресХранилища)
	...
	Если ЗначениеЗаполнено(СостояниеВыгрузкиЗагрузки) Тогда
		...
		СостояниеЗагрузки.ПредставлениеСостояния = ВыгрузкаЗагрузкаДанныхСлужебный.ПредставлениеСостоянияВыгрузкиЗагрузкиОбластиДанных(
			СостояниеВыгрузкиЗагрузки);
		СостояниеЗагрузки.ПроцентЗавершения = ВыгрузкаЗагрузкаДанныхСлужебный.ПроцентЗавершенияВыгрузкиЗагрузкиОбластиДанных(
			СостояниеВыгрузкиЗагрузки);
...
Процедура ЗапуститьЗагрузкуДанныхНаСервере()
	...
	Если ИспользоватьМногопоточность() Тогда
		УстановитьМонопольнуюБлокировку();
		СнятьМонопольнуюБлокировку();
	КонецЕсли;
	
	ИмяФайлаНаСервере = ?(ЭтоВебКлиент Или Не ФайлДанныхДоступенНаСервере,
		ФайлыБТССлужебный.ПолноеИмяФайлаВременногоХранилища(ИмяФайлаВременногоХранилища),
		ФайлДанных);
...
Процедура ВосстановитьРасширенияИнформационнойБазы(ИмяФайлаНаСервере)
	...
	ВыгрузкаЗагрузкаДанныхСлужебный.ВосстановитьРасширенияПередЗагрузкойДанных(ДанныеРасширений);	
...
Функция РасширенияДляВосстановления(ИмяФайлаНаСервере, ОчиститьОкноСообщений)
	...
	ЧтениеАрхива = Новый ЧтениеФайлаАрхива(ИмяФайлаНаСервере);

	Попытка
		РасширенияДляВосстановления = РасширенияДляВосстановленияВнутр(ЧтениеАрхива, ОчиститьОкноСообщений);
	Исключение
		ЧтениеАрхива.Закрыть();
		ВызватьИсключение;
	КонецПопытки;
...
Функция КаркасыРасширенийДляВосстановления(ИмяФайлаНаСервере)
	...
	Попытка
		РасширенияДляВосстановления = КаркасыРасширенийДляВосстановленияВнутр(ЧтениеАрхива);
	Исключение
		ЧтениеАрхива.Закрыть();
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ЗавершитьЗагрузкуНаСервере()
	...
	ЗагрузкаПрерванаВРезультатеОшибки = Ложь;
...
Процедура УдалитьВременныеДанныеПослеЗагрузки(ИмяФайлаХранилища)
	...
	ФайлыБТССлужебный.УдалитьФайлВременногоХранилища(ИмяФайлаХранилища);
...
Процедура ПоказатьОшибкиПредупреждения(ПараметрыУведомления)
	...
	ОшибкиПредупрежденияТекст = ПараметрыУведомления.Текст;
	Картинка = ?(ПараметрыУведомления.ЭтоОшибка, БиблиотекаКартинок.ДиалогСтоп, БиблиотекаКартинок.ДиалогВосклицание);

    Если Не ПерезапуститьПрограммуПриЗакрытии Тогда
		Элементы.Продолжить.Заголовок = "Повторить";  
...
&НаКлиенте
Процедура ПродолжитьПрерваннуюЗагрузкуПриНеобходимости()
...
&НаСервере
Процедура ЗаполнитьПараметрыПрерваннойЗагрузки()
	...
	Если Не ПараметрыЗагрузки.Свойство("ЗагружатьРасширенияКонфигурации") Тогда
	
		УстаревшиеПараметры = Новый Структура("НеЗагружатьРасширенияКонфигурации", Ложь);
		ЗаполнитьЗначенияСвойств(УстаревшиеПараметры, ПараметрыЗагрузки);
		
		ЗагружатьРасширенияКонфигурации = Не УстаревшиеПараметры.НеЗагружатьРасширенияКонфигурации;
...
Процедура ЗаполнитьРеквизитыЭлементыФормы()
	...
	ЗагружатьРасширенияКонфигурации = Истина;
	ДополнительныеПараметрыОткрыта = Ложь;
	...
	Если ЭтоФайловаяИнформационнаяБаза Или Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		Элементы.СписокПользователейЗавершитьСеансы.Видимость = Ложь;
	КонецЕсли; 
...
&НаСервере
Процедура СброситьДанныеФормы()
	
	ЗагрузкаПрерванаВРезультатеОшибки = Ложь;
	ФайлДанных = "";
	ТекстПредупреждения = "";  
	Для Инд = 0 По ВсегоПредупреждений Цикл
		ДекорацияУдалить = Элементы.Найти("ДекорацияПредупреждение_" + Формат(Инд, "ЧГ=0;"));
		Если ДекорацияУдалить = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Элементы.Удалить(ДекорацияУдалить);
	КонецЦикла;
	ВсегоПредупреждений = 0;
	Элементы.ГруппаОжиданиеПредупреждения.Видимость = Ложь;
...
&НаСервере
Процедура УстановитьМонопольнуюБлокировку()
...
&НаСервере
Процедура СнятьМонопольнуюБлокировку()
...
&НаСервереБезКонтекста
Процедура УстановитьКрасныйЦвет(СвойствоЦвет)
	
	СвойствоЦвет = ЦветаПалитры.Красный;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СброситьЦвет(СвойствоЦвет)
	
	СвойствоЦвет = Новый Цвет();
