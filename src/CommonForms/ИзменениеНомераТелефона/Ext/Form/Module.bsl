...
Процедура ФайлЗаявленияНажатие(Элемент, СтандартнаяОбработка)
    ...
    ПараметрыДиалога.Заголовок = НСтр("ru = 'Выберите файл Заявления'");
...
&НаКлиенте
Процедура ФайлЗаявлениеНажатиеПослеПомещенияФайла(Результат, ВходящийКонтекст) Экспорт
	
	Если ТипЗнч(Результат) = Тип("ОписаниеПомещенногоФайла") Тогда
		Файл = Результат.СсылкаНаФайл.Файл;
		ФайлЗаявления = Результат.СсылкаНаФайл.Имя;
		АдресФайлаЗаявления = Результат.Адрес;
	КонецЕсли;
...
Процедура ОтправитьЗаявление(Команда)
	...
	ПараметрыФормы.Вставить("Владелец", "ИзменениеНомераТелефона");
...
Процедура РезультатВыбораПользователя(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора = КомандаОтправитьЗаявление() Тогда
		ПодготовитьИОтправитьЗаявление();		
	КонецЕсли;
	
КонецПроцедуры    

&НаКлиенте
Процедура ПодготовитьИОтправитьЗаявление()  
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаКодПодтверждения.Видимость = 
		...
		И Не Форма.ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено;  
    Элементы.ОтменитьПроверкуТелефона.Видимость = 
		Форма.ПроверкаТелефонДляПаролей.ВыполняетсяПроверка 
		И Не Форма.ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено;  
